...
describe('dev-all ports', () => {
  ...
  it('automatically seeds the canonical empty demo accounts', () => {
    ...
    expect(accountSeed).toContain("collection('municipalities').doc('daet')")
  ...
  it('starts emulators and apps with one explicit demo project id', () => {
    ...
    expect(script).toContain('VITE_FIREBASE_API_KEY')
    expect(script).toContain('VITE_FIREBASE_AUTH_DOMAIN')
    expect(script).toContain('VITE_FIREBASE_APP_ID')
    expect(script).toContain('VITE_FIREBASE_MESSAGING_SENDER_ID')
    expect(script).toContain('VITE_FIREBASE_STORAGE_BUCKET')
    expect(script).toContain('VITE_FIREBASE_DATABASE_URL')
