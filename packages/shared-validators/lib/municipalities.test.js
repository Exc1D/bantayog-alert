...
describe('updateMunicipalityContactInputSchema', () => {
    ...
    it('rejects missing fields and unknown keys', () => {
        expect(updateMunicipalityContactInputSchema.safeParse({ municipalityId: 'daet' }).success).toBe(false);
        expect(updateMunicipalityContactInputSchema.safeParse({ ...VALID_INPUT, extra: true }).success).toBe(false);
    });
});
...
//# sourceMappingURL=municipalities.test.js.map