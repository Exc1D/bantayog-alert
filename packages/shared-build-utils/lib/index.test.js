...
describe('assertNoEmulatorInProduction', () => {
    ...
    it('does not throw when emulator is undefined', () => {
        expect(() => {
            assertNoEmulatorInProduction('build', 'production', undefined, 'test-app');
        }).not.toThrow();
    });
    it('does not throw in dev mode even with emulator=true', () => {
        expect(() => {
            assertNoEmulatorInProduction('serve', 'development', 'true', 'test-app');
        }).not.toThrow();
    });
    it('does not throw during preview', () => {
        expect(() => {
            assertNoEmulatorInProduction('build', 'staging', 'true', 'test-app');
        }).not.toThrow();
    });
});
