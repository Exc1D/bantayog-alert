...
- **Canvas/map markers (Leaflet `divIcon`) are invisible to keyboards and screen readers.** The robust pattern is a parallel, focusable DOM list that mirrors the map content. Render it as `opacity-0 pointer-events-none focus-within:opacity-100 focus-within:pointer-events-auto` so it appears on Tab but is hidden during mouse interaction. Each list item should have a rich `aria-label` describing the pin's meaning (type, severity, location). Clicking a list item should trigger the same handler as clicking the map pin, so behavior is identical for all input modalities.
