...
- Client fan-out of a per-item moderation callable (hide-all-by-author) is fine below the callable's rate limit (60/min here); the loop must tolerate single-item failures (the wrapper catches internally) so one bad post doesn't strand the rest. Tolerating failures internally isn't enough on its own: the original "Hide all by author" handler called the fan-out with `void asyncFn(...)` and closed the `ConfirmationModal` (`setConfirmHideAuthor(null)`) on the same tick, before any await resolved — the modal's `confirmLoading` gate existed but nothing set it, so per-post failures were swallowed silently and the operator saw instant fake success. Fix: `await` the fan-out fully, track `confirmLoading` for its duration, and report partial failure counts instead of a blanket success message.
- A rules `create: if false` flip that retires a validator function (e.g. `validSituationUpdate`) leaves the function itself dead — its constraints move into the replacement callable's zod schema, but the rules function has no more callers. Remove it in the same PR the flip lands in, not as a follow-up; a bot/reviewer will flag the orphan otherwise.
- "Report partial failure counts instead of a blanket success message" (previous entry) is only half the fix if success/error banners are two separate pieces of state: a per-item success toast set on an earlier-succeeding iteration of the same fan-out survives into the partial-failure branch unless the failure path explicitly clears it. Any handler that sets both an error and a success signal across a loop must null out the other on every exit path, not just set the one it means to show.
