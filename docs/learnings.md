...
## Admin Desktop UX Hardening (2026-05-26)

- **Modal forms MUST be wrapped in `<form>` with a `<button type="submit">`.** `DeclareAlertModal` had a generic `<button type="button">` with an `onClick` handler instead of a submit button inside a form. This broke Enter-key submission, which is a critical usability failure for fast emergency actions. The fix: wrap all fields + footer buttons in `<form onSubmit={handleSubmit}>`, change the primary action to `type="submit"`, and add `e.preventDefault()` in the submit handler. This is the standard, robust pattern for form submission.
...
- **`useFocusTrap` must not steal focus from elements already inside the container.** Tests that `focus()` a specific element before asserting keyboard behavior will fail if the focus trap immediately shifts focus to the first focusable on activation. Guard with `if (!containerRef.current?.contains(document.activeElement))` before auto-focusing.
- **Multiple `role="status"` elements on the same page break `screen.getByRole('status')`.** `LiveIndicator` dot + `SuccessBanner` both use `role="status"`. Prefer querying by visible text (`getByText`) or add `data-testid` to disambiguate, rather than relying on role uniqueness.
- **Responder App offline indicator**: When responders operate in disaster zones, `navigator.onLine` is the first sign of trouble. Adding a reactive offline dot in the header gives immediate feedback without requiring a full "you are offline" banner. (2026-05-26)
- **Input `maxLength` without a visible counter is a trap.** Users hit the 2000-char Firestore limit silently. Adding `maxLength={2000}` + a live `charCount` label prevents truncation errors and gives users a clear contract. (2026-05-26)
- **`prefers-reduced-motion` must target the specific animated element.** Global `@media` in `globals.css` only catches transitions/animations on universal selectors. For `@keyframes` on named classes (e.g. `ringActive`), add a dedicated `@media` block scoped to that class. (2026-05-26)
- **Skip links should use `position: absolute` + `top: -40px` → `top: 8px` on `:focus` with a high `z-index`.** This makes them visually hidden until keyboard-tabbed, matching standard a11y patterns. (2026-05-26)
- **Adding `loading` to a live-listener hook is risky** — existing tests destructure the return and will crash if a new field is added. When extending hook contracts, always: (1) check all destructuring consumers, (2) add default values to all mock factories, (3) keep the new field optional in test state objects. (2026-05-26)
- **Long titles on dispatch cards need `overflow: hidden` + `text-overflow: ellipsis` on the card header.** Flooding or structural failure incidents may have verbose generated titles. (2026-05-26)
- **Destructuring-based ESLint rules make `useState(() => !!uid)` hard to type.** `loading: useState(!!uid)` would run every render. `useState(() => !!uid)` is the correct lazy-initializer pattern, but the `setLoading` setter must be available in `onSnapshot` callbacks. (2026-05-26)
- **Skeletons need `aria-hidden="true"`** — they are purely decorative placeholders and should never be read by screen readers. Pair with a visible "Loading…" `aria-live="polite"` heading for SR users. (2026-05-26)
- **Onboarding hints under the timeline help first-time responders** — many emergency responders have never used a mobile dispatch app before. A single line telling them to "tap the action button below" removes confusion about why the timeline is static. (2026-05-26)
- **Backdrop blur (`backdrop-blur-sm`) is banned per `PRODUCT.md` anti-references.** Easy to miss during code review because Tailwind classes compress well; automated scanning does not catch project-specific semantic bans. Audit all `backdrop-blur` occurrences when the anti-reference changes.
- **Map overlay `absolute right-4` collides with `TriagePanel` at `right-0`.** Shifting to `right-[400px]` when panel is open avoids overlap. Prefer dynamic class composition over z-index escalation.
- **`useEffect` depending on `reportDocs` (an array reference) causes spurious re-runs.** Firestore listener returns a new array reference on every snapshot. Use `reports.length` or a stable string key instead of the raw reference in the dependency array.
- **`useKeyboardShortcuts` must be guarded when a modal/panel is open.** If `?` opens `HelpModal` while `TriagePanel` is focused, the shortcut handler races with `useFocusTrap`. Guard shortcuts with `if (!selectedReportId)` or similar state checks.
- **Empty-state `backdrop-blur-sm` also violates `PRODUCT.md`.** The "No active incidents" overlay used the same banned class. Audit all `backdrop-blur` occurrences in `apps/admin-desktop` to remove them.
- **`pointer-events: none` with child `pointer-events: auto` is a brittle interaction hack.** Gaps between children let clicks pass through to underlying elements (e.g., map). Remove the parent `none` and let normal event bubbling handle it; `absolute` positioning already prevents map interference.
- **`ActionErrorBanner` should be reused instead of inline raw HTML.** DashboardPage has a proper component; MapPage had divergent inline styling. Centralize for consistent error UX across all pages.
- **`useKeyboardShortcuts` must include the same navigation shortcuts across all pages.** DashboardPage has `r`, `d`, `f`, `?`, `Esc`. MapPage must match the common subset (`d`, `f`, `?`, `Esc`) plus page-specific extras (`↑`/`↓` for incident list). Otherwise power users lose muscle memory when switching tabs.
- **`lastUpdatedAt` tracking must use content fingerprint, not array reference or length.** Firestore listeners return new array references on every snapshot. `reports.length` misses edits to existing docs. `reports.map(r => r.id + ':' + r.status + ':' + r.updatedAt).join('|')` is a robust, deterministic fingerprint that catches all meaningful changes without spurious updates on reference churn.
...
- **2026-05-26: Subscription callables need user-keyed rate limits, not token-keyed limits.** FCM tokens can rotate or be spammed by a compromised client. Use a quota key like `subscribeToAlerts:{uid}` so token churn cannot bypass the throttle, and preserve `HttpsError` in callable wrappers so `resource-exhausted` reaches clients instead of becoming `internal`.
- **2026-05-26: Paired FCM topic callables must be hardened symmetrically.** If `subscribeToAlerts` has App Check, token ownership, per-user rate limiting, and `HttpsError` preservation, then `unsubscribeFromAlerts` needs the same protections. Attackers abuse whichever sibling is cheaper or less guarded.
- **2026-05-26: Public or anonymous-capable callables must never return raw unexpected `err.message`.** `requestLookup` can be called by unauthenticated users and previously surfaced backend exception text as `internal`. Log server-side details, return a fixed client message, and add a wrapper-level regression test.
- **2026-05-26: Authenticated signed-URL callables also need sanitized internal failures.** `requestUploadUrl` previously returned raw storage signing exceptions to clients. Treat service-account, bucket, and signing errors as server-side details only; clients get a fixed public message.
- **2026-05-26: Tests that intentionally trigger server logging should silence that log locally.** Wrapper tests for sanitized `internal` errors should `vi.spyOn(console, 'error')` inside the test and restore in `finally`; otherwise passing tests produce scary stderr and hide real failures.
- **2026-05-26: Smoke checks need explicit bucket config and per-check timeouts.** `smoke-test-prod.ts` used `storage.bucket()` without initializing `storageBucket`, then hung when staging credentials were unavailable. Use `STORAGE_BUCKET` or `${project}.appspot.com`, keep bucket creation inside `try`, and wrap each external check in a timeout so readiness gates fail loudly.

## Test Hardening (2026-05-26)

- **Root `vitest.config.ts` must include every `shared-*` package explicitly.** `packages/shared-data` tests were silently ignored because the root config only listed `shared-validators`, `shared-firebase`, `e2e-tests/fixtures`, and `scripts`. Always expand the `include` array when adding tests to a new shared package; the package-level vitest config is not consulted when running from the root.
- **`packages/shared-data` constants need regression tests too.** Even "trivial" constant arrays are production-critical (municipality list drives analytics + mass-alert scope). A 3-line test is cheaper than a production incident caused by an accidental deletion.
- **Responder telemetry hook was untested despite being a critical disaster path.** GPS tracking, battery throttling, and RTDB/Firestore writes are all tested now. Any new `useEffect`-driven side-effect hook should have at least one test verifying lifecycle (start/stop) and one verifying throttling/correctness.
- **k6 load tests must stay out of the PR gate but in a scheduled workflow.** PR CI is too slow and variable for load tests. A nightly cron isolates infrastructure noise and gives a baseline. Keep `workflow_dispatch` so engineers can trigger ad-hoc runs against staging before a major deploy.
- **e2e full-loop proof belongs in CI, not just local dev.** `proof-local` is the canonical path: if it breaks locally, it will break in CI. Adding it as a job that depends on build + emulator tests prevents shipping broken end-to-end flows.
