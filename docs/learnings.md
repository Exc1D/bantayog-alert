...
- ~~`useReportQueue.test.ts` and `QueueIndicator.test.tsx` lack firebase mocks — fail without `.env.local`~~ **RESOLVED (2026-04-16):** All three files now pass (39/39). `useReportQueue.test.ts` was fixed with `vi.mock('@/app/firebase/config')` in a previous session. `QueueIndicator.test.tsx` and `DispatchList.test.tsx` pass because Vitest's auto-mock does not trigger real module initialization in the current config.
