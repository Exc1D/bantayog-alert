...
- Firebase multisite deploy needs both pieces: a `firebase.json` hosting entry with `target: "responder"` and a `.firebaserc` target mapping to the Hosting site. `firebase target:apply` can create/update the `.firebaserc` mapping, but it does not add the required `firebase.json` hosting block. If the site itself does not exist, create it first with `firebase hosting:sites:create <site-id> --project <project-id>`, then deploy the scoped target.
- Broad shared package peer ranges can bundle duplicate singletons. `@bantayog/shared-ui` with loose React/router/Firebase peers pulled stale local installs into the responder build, causing React `useState` null-dispatcher crashes, Firebase `Service database is not available`, and router-context errors. Keep singleton peer ranges aligned with app versions and verify source maps contain one React/router/Firebase version before deploy.
- Do not build staging from `.env.local` with `VITE_USE_EMULATOR=true`. Vite embeds it into the static artifact and the deployed app will call `localhost:8081`. For staging Hosting builds, override with `VITE_USE_EMULATOR=false` and smoke-test the live URL for failed `localhost` requests.
- Guard production responder builds in `vite.config.ts`: fail closed when the effective env still has `VITE_USE_EMULATOR=true`. This catches the broken-artifact case before `firebase deploy` can publish it; staging/prod builds must pass `VITE_USE_EMULATOR=false` explicitly when a local `.env.local` is present.
- PWA manifest icons must exist in `public/` before deploy. A manifest can pass build while pointing at missing `/icon-192.png`; verify the deployed icon URL returns `200 image/png`.
...
- Citizen-facing municipality contact docs are public metadata. Keep `municipalities/{municipalityId}` readable by unauthenticated users and writable only by server/callable paths; pair the rule with `public-collections.rules.test.ts` so `useMunicipalityContact` does not regress to DEFAULT_CONTACT warnings in staging.
