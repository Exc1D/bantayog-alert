...
- Closed three coderabbitai test-coverage gaps: `createSituationUpdate` callable wrapper tests (unauthenticated/inactive/malformed/rate-limited/success, 5 new cases mirroring `submit-report-feedback.callable.test.ts`), a `situationFlagCounter` trigger error-path test (transaction rejection → logged + rethrown, using the `vi.importActual` spread-mock pattern from `close-report.unit.test.ts` to stub `logDimension` only), and an `itif('rejects unauthenticated creation attempts', ...)` Firestore rules test for `situation_updates` create.
- Bumped `createSituationUpdate` `maxInstances` 10→50 to match the citizen-surge-write convention (`submit-citizen-report`, `submit-report-feedback`).
- Removed the dead `validSituationUpdate()` Firestore rules function (38 lines) from both `firestore.rules` and `firestore.rules.template` — flagged by coderabbitai as unreachable since the 2026-07-09 entry flipped `situation_updates` `allow create` to `if false` without removing the now-unused validator; its constraints already live on in the `createSituationUpdate` callable's zod schema. User-approved removal (explicit AskUserQuestion sign-off) per CLAUDE.md §6/§8.4 rules-file gate.
- Verification: full Firestore/RTDB/Storage rules emulator suite 28 files/221 tests (was 27/213 pre-PR — 8 new tests across this PR); functions unit 13/13 for the two touched files; admin-desktop feed-page 14/14; `tsc --noEmit` clean for admin-desktop and functions; eslint clean on all 8 changed files; `git diff --check` clean. Not deployed — same soak requirement as the 2026-07-09 entry applies to the rules-file change.

## 2026-07-09 - Typhoon-readiness feed moderation hardening (with rules)

- Closed the three feed-abuse gaps found in the typhoon-readiness assessment, user-approved including rules. Slice A (admin window): `FeedPage` now shows ALL in-scope situation posts (was `.slice(0, 10)` vs the citizen 100-limit), sorted flagged-first (`reportedCount` desc, then `createdAt` desc), shows `authorUid` on each card, and adds a confirmed "Hide all by author" that sequentially fans out the existing per-post `setCitizenContentVisibility` callable over that author's visible public posts (`// ponytail:` bulk endpoint only matters past the 60/min moderation rate limit).
- Slice B (flag loop): new `situationFlagCounter` `onDocumentCreated` trigger on `situation_updates/{updateId}/reports/{reportId}` transactionally increments the parent `reportedCount` with concrete read-then-write (no `FieldValue.increment` — rules-unit-testing incompatibility). Citizen flags now actually surface in the admin flagged-first sort; before this, nothing ever wrote `reportedCount`.
- Slice C (rate limit): new `createSituationUpdate` callable (region `asia-southeast1`, `getCitizenCallableCorsOrigins()`, App Check, auth parity with the removed rule: any active signed-in account, no role gate) with inline zod mirroring `validSituationUpdate` and `checkRateLimit` 5 posts/600 s/uid → `resource-exhausted`. Citizen `createSituationUpdate` service swapped from direct `addDoc` to the callable (keeps `ensureSignedIn` + `normalizeInput`, validates `updateId` in the response). Rules: `situation_updates` `allow create` flipped to `if false` in BOTH `firestore.rules` and `firestore.rules.template`; flags subcollection unchanged; the now-dead `validSituationUpdate` rules function left in place (flip-only diff — its constraints live on in the callable zod schema).
- Deploy-order note (NOT deployed — §8.4 soak applies): the rules flip must not reach staging/prod before the `createSituationUpdate` function does, or citizen posting bricks. Deploy functions first, rules second.
- Verification: red-first at every step (feed-page 2 new tests failed on the old cap/no-button; callable test failed on missing module; citizen test failed on the addDoc path; rules denial test failed against pre-flip rules). Green: feed-page 13/13, functions unit 7/7 (flag counter 3 + callable 4), citizen situation-updates + FeedTab 25/25, emulator rules suite 8/8 post-flip; admin/citizen/functions `tsc --noEmit` clean; eslint clean on all changed files.
...
- Slice R — responder lifecycle on `/dispatches`, gated behind `canManageResponders = claims?.role === 'agency_admin'`: `suspendResponder`, `revokeResponder`, and `bulkAvailabilityOverride` (as a single-uid `off_duty` override). Actions live in the expanded `ResponderAvailabilityRow` (new `ResponderRowActions` interface, off-duty/suspend/revoke buttons; `ResponderAvailabilityPanel` passes `rowActions` through), each fired through a `ConfirmationModal` (off-duty `primary`, suspend/revoke `danger`) and `withRetry`+idempotency key. The prior entry's deferral was gated on Auth-claim propagation for suspend/revoke — that gap was already closed in PR #212 (2026-06-14, `setCustomUserClaims` after the Firestore status change), so UI wiring is no longer blocked on that axis; no deploy here, so §8.4's soak requirement is untouched. `// ponytail:` note on the off-duty override: no "bring back to available" path because the fleet list is available-only filtered — that needs a roster dataset that doesn't exist yet. 4 files: `callables.ts` (three wrappers + `AvailabilityStatus`), `ResponderAvailabilityRow.tsx`, `ResponderAvailabilityPanel.tsx`, `DispatchMonitorPage.tsx` (state/handlers/2 modals).
- Slice T — report lifecycle on `/triage`, gated behind `canManageLifecycle = role municipal_admin || provincial_superadmin`: `closeReport`, `reopenReport`, `mergeDuplicates`. `canManageLifecycle` widens the rendered report set and status-filter options to include `resolved`/`closed` (default `all` filter still excludes them so the queue stays clean). Per-row Close (Archive icon, `resolved`) / Reopen (RotateCcw, `closed`) buttons plus a header "Merge duplicates" button (enabled at ≥2 selected). One discriminated `pendingLifecycle` state feeds ONE `ConfirmationModal` for both close/reopen; merge uses a second modal with a Primary-report `<select>` (`// ponytail:` primary defaults to the first selected row, operator repicks). `mergeDuplicates` handles both `{success:true; mergedCount}` and `{success:false; errorCode}` union arms. Reopen reason is required — `ConfirmationModal` has no `confirmDisabled`, so the handler guards an empty reason as a silent no-op and the placeholder states the requirement. 3 files: `callables.ts` (shared), `TriageQueueTable.tsx`, `TriagePage.tsx` (+ `TriagePage.test.tsx`).
- Full command-authority set now complete: cancelDispatch (prior) + these six = all seven dispatch/report/responder callables the runbook listed as backend-only are UI-invoked. The remaining backend-only callables (user suspend/revoke, TOTP reset, createUser, erasure, retention/legal-hold) are separate concerns, unchanged.
- Verification: `DispatchMonitorPage.test.tsx` 21/21 (Slice R interaction tests added), `TriagePage.test.tsx` 23/23 (merge/close/reopen + role-gate-hides-controls tests added), admin-desktop `tsc --noEmit` clean, `eslint src` clean.

## 2026-07-05 - Admin cancelDispatch wiring (command-authority slice)

- Wired the existing backend `cancelDispatch` callable into the Admin `/dispatches` surface: a per-row "Cancel dispatch" button on cancellable-status rows (`pending`/`accepted`/`acknowledged`/`en_route`/`on_scene`, mirroring backend `CANCELLABLE_FROM_STATES`) opens a `ConfirmationModal` with a reason `<select>` (`admin_error` default), routed through the same `executeDispatchCommand`/`withRetry`/idempotency-key/`ActionErrorBanner` retry infrastructure as escalate/assign. 4 files: `callables.ts` (new wrapper + `CancelDispatchReason` type), `DispatchLifecycleTable.tsx` (optional `onCancelDispatch` + status gate), `DispatchMonitorPage.tsx` (cancel state/handlers/modal, `cancel` arm on the `FailedDispatchCommand` union), plus a new interaction test. Working-tree only, no commit, no rules/schema/deploy.
- Ponytail scope decision on the admin-desktop "implement items 2 and 3" request: shipped `cancelDispatch` only — the clean, reversible, no-Auth-claim gap that reuses all existing dispatch infra. Deferred with reasons: `suspendResponder`/`revokeResponder` mutate Firebase Auth custom claims (§8.4 risky-change; ledger's "verify suspend/revoke propagate to Auth before shipping") AND target the available-only fleet list (a roster dataset never built); `bulkAvailabilityOverride` needs the same roster surface; `mergeDuplicates` needs net-new multi-select UI; `closeReport`/`reopenReport` are a separate report-lifecycle concern. Await explicit go-ahead before building the remaining six.
- Verification: `DispatchMonitorPage.test.tsx` 20/20 (new cancel test asserts payload `{ dispatchId, reason: 'duplicate_report', idempotencyKey }` and the success banner), admin-desktop typecheck clean, lint clean.

## 2026-07-03 - Audit-fix implementation (handoff findings, non-rules subset)

- Implemented 11 non-rules findings from the 2026-07 audit handoff, working-tree only (no commits), grouped file-disjoint for a 7-PR split. Auth/claims: `suspendUserCore`/`revokeUserCore` now push `accountStatus` into custom claims and revoke refresh tokens via a shared `invalidateUserSessions` helper; `resetUserTotpCore` revokes refresh tokens after MFA clear (unit tests 15/15). Dispatch: `escalateDispatchCore` gained a terminal-status guard (resolved/cancelled/superseded), recomputes `acknowledgementDeadlineAt` from report severity via `getRedispatchDeadlineMs`, sets `statusUpdatedAt`, and moved all FCM/event side effects inside `withIdempotency`; `monitor-dispatch-deadlines` now tallies needs-admin alerts per municipality with concrete writes instead of Admin `FieldValue` transforms (emulator tests 12/12 across both files — the monitor file had been silently skipping all tests via a legacy `beforeAll` guard).
- Config: functions `deploy:staging`/`deploy:prod` scripts and `sw.js` project-ID resolution now target live project IDs (`bantayogalert-staging`, `-stg` host detection); retired `bantayog-alert-staging` references purged from staging seed/reset/incident scripts and `terraform.tfvars`; root `test` script runs root vitest before turbo. Admin-desktop: hoisted idempotency-key generation before `withRetry` in MapPage (verify/reject/dispatch), DashboardPage (re-dispatch), and 4 additional FeedPage sites found by grep (39/39 focused tests). `createResponderCore` rejects a supplied `municipalityId` outside caller jurisdiction (new unit test 2/2). `ProtectedRoute` fixed to check `claims.accountStatus === 'active'` (the `active` boolean never existed) and restored the commented-out municipality-scope guard (shared-ui 15/15); `AccountStatus` gained `'revoked'`.
- Rules findings (#1 system_config world-readable, #8 RTDB agency_admin shared_projection no municipality match, #9 bare `uid()` updates without `isAuthed()`, #13 situation_updates report-flag create missing accountStatus, #18 emergencies blanket authed read) were NOT applied — full diffs presented for explicit approval per §8.4.
- Verification: root typecheck 10/10, root lint 10/10, root vitest 24 files / 223 tests, functions `tsc --noEmit` clean, focused suites green as listed above.
- Follow-up after explicit user "proceed": applied #1/#9/#13 to both `firestore.rules` and `firestore.rules.template` and #8 to `database.rules.json`. #1 deviated from the presented diff on purpose: both `useVersionGate` hooks read `system_config/min_app_version` + `update_urls` pre-auth and fail open on permission-denied, so a blanket `isAuthed()` would silently disable version gating — instead only those two doc IDs stay public and everything else in `system_config` is private. #18 left unchanged (flag-only, needs product intent). One rules test updated red-first for #13 (`accountStatus: 'active'` claim on the moderation-flag fixture) plus a new suspended-account denial test. Full Firestore + RTDB rules suites under emulators: 30 files / 250 tests passing. Still working-tree only — no deploy; staging soak per §8.4 required before prod.
