...
- Turned the resolved `/grill-me` design-decision set for `docs/bantayog-alert-citizen-pwa-spec.md` (v2.0) into a phased `cpwa-` slice backlog under `docs/agent-tasks/`, matching the existing `3c-00`/`3b-09` index+slice convention: `cpwa-00` index plus nine ranked slices `cpwa-01`…`cpwa-09`.
- Headline resolved decisions captured: bottom nav becomes `Home · Map · Report · Feed · Profile` (Alerts leaves the nav, reached via a Home-header bell; `/alerts` route survives); Home becomes `/` with a dynamic "Your Local Brief" hero over a stable secondary stack; Map demotes to `/map`; the §9 Response Thread becomes the primary tracking surface at `/track/:id`; one shared status registry (`cpwa-01`) gives every surface the same icon+label+token vocabulary while keeping the §5 axes separate. The product owner's Home-only override of the §14.2 motion ban is encoded as the M-A/M-B/M-C motion layer (`cpwa-06`) with a mandatory reduced-motion fallback.
- Sequencing: `cpwa-01` (status registry) and `cpwa-02` (IA/routing spine) are P0 and land before the surface slices; `cpwa-07` (Response Thread route) is also P0 as the structural heart. Each slice is frontend-only, ≤3 files, red-first, preserves the intentional teal `#0d7377` palette, and changes zero rules/indexes/schema.
- Open seams recorded in the index rather than guessed: §6.14 "deliberate ending" copy settles in `cpwa-05`; weather source (B6) ships as a truth-gated empty slot in `cpwa-04` until a backing service exists; the Map alert-zone layer in `cpwa-09` defers if alert docs carry no affected-area geometry (that would be a backend escalation, not a slice). Explicitly rejected: full-screen emergency takeover, looping hazard backgrounds, tracking-in-Map-sheet, and a second status component per surface.
- Documentation-only slice: no code, runtime, rules, indexes, schema, dependency, or deploy changes. The slices are specs for later red-first execution, each on its own branch.
