...
- Updated `docs/mvp-readiness.md` to mark "Real staging/prod deployment proof" as satisfied (verified against real staging project).
- **Real staging execution achieved:** Service account key used to seed 10 reports, 10 lookups, 5 alerts, 1 dispatch into `bantayog-alert-staging`. Both `pnpm staging:smoke-proof` and `pnpm staging:e2e-proof` passed against real staging.
