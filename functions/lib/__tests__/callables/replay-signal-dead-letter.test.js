...
const mockPagasaSignalPollCore = vi.hoisted(() => vi.fn().mockResolvedValue({ status: 'updated', scraperDegraded: false }));
...
function createMockDb(deadLetters) {
    ...
    const collectionFn = vi.fn((collectionName) => {
...
describe('replaySignalDeadLetter', () => {
    it('replays hazard signal projection dead letters for provincial superadmins', async () => {
        ...
        const invokeReplay = replaySignalDeadLetter;
        ...
        expect(mockDb._updateFn).toHaveBeenCalledWith({
...
//# sourceMappingURL=replay-signal-dead-letter.test.js.map