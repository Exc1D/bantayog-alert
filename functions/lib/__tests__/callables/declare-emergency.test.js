...
describe('declareEmergencyCore', () => {
    ...
    it('throws ZodError for empty hazardType', async () => {
        await expect(declareEmergencyCore(mockDb, { ...validInput, hazardType: '' }, { uid: 'admin-1' })).rejects.toThrow(ZodError);
    });
    it('throws ZodError for empty municipalityIds', async () => {
        await expect(declareEmergencyCore(mockDb, { ...validInput, affectedMunicipalityIds: [] }, { uid: 'admin-1' })).rejects.toThrow(ZodError);
    });
