...
import { markDispatchUnableToComplete, markDispatchUnableToCompleteCore, } from '../../callables/mark-dispatch-unable-to-complete.js';
...
async function seedReport(env, reportId, status) {
    await env.withSecurityRulesDisabled(async (ctx) => {
        const db = ctx.firestore();
...
describe('markDispatchUnableToCompleteCore', () => {
    ...
    it('rejects when dispatch is not active', async () => {
        ...
        await testEnv.withSecurityRulesDisabled(async (ctx) => {
            const db = ctx.firestore();
            await expect(markDispatchUnableToCompleteCore(db, {
                dispatchId: 'dispatch-2',
                reason: 'Cannot reach',
                idempotencyKey: crypto.randomUUID(),
                actor: { uid: 'r1', claims: { role: 'responder', municipalityId: 'daet' } },
...
//# sourceMappingURL=mark-dispatch-unable-to-complete.test.js.map