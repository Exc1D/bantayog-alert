...
import { seedReportAtStatus, seedActiveAccount, seedResponderDoc, seedResponderShift, staffClaims, } from '../helpers/seed-factories.js';
...
describe('dispatchResponderCore', () => {
    ...
    it('sets acknowledgementDeadlineAt according to severity', async () => {
        ...
        const dispatch = (await db.collection('dispatches').doc(result.dispatchId).get()).data();
        expect(dispatch.acknowledgementDeadlineAt.toMillis() - now.toMillis()).toBeCloseTo(5 * 60 * 1000, -3);
    });
});
