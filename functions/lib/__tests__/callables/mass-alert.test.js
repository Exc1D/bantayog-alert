...
import { massAlertReachPlanPreviewCore, sendMassAlertCore, requestMassAlertEscalationCore, forwardMassAlertToNDRRMCCore, } from '../../callables/mass-alert.js';
...
async function seedResponder(id, hasFcmToken) {
...
describe('massAlertReachPlanPreview', () => {
    it('rejects citizens and responders', async () => {
        const result = await massAlertReachPlanPreviewCore(adminDb, {
...
describe('sendMassAlert', () => {
    ...
    it('is idempotent', async () => {
        ...
        const r1 = await sendMassAlertCore(adminDb, {
            reachPlan: {
                route: 'direct',
                fcmCount: 5,
                smsCount: 3,
        ...
        const r2 = await sendMassAlertCore(adminDb, {
            reachPlan: {
                route: 'direct',
                fcmCount: 5,
                smsCount: 3,
...
describe('requestMassAlertEscalation', () => {
    ...
    it('does not send responder FCM during escalation (reviewer channel TBD)', async () => {
        ...
        mockFcm.mockClear();
        await requestMassAlertEscalationCore(adminDb, {
            message: 'Alert',
            targetScope: { municipalityIds: ['daet'] },
...
describe('forwardMassAlertToNDRRMC', () => {
    async function createPendingRequest(id) {
        await testEnv.withSecurityRulesDisabled(async (ctx) => {
...
//# sourceMappingURL=mass-alert.test.js.map