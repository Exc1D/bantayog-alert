...
import { unsubscribeFromAlertsCore } from '../../callables/unsubscribe-from-alerts.js';
describe('unsubscribeFromAlertsCore', () => {
    ...
    it('propagates messaging errors', async () => {
        ...
        await expect(unsubscribeFromAlertsCore({ token: 'bad-token', actor: { uid: 'user-123' } })).rejects.toThrow('messaging error');
    });
});
