 ...
 */
async function seedReportAtStatusJS(db, reportId, status, opts = {}) {
 ...
 */
async function seedResponderOnShift(db, rtdb, responderUid, municipalityId) {
...
beforeAll(async () => {
    ...
    const setupDb = testEnv.unauthenticatedContext().firestore();
...
describe('Phase 4a Acceptance', () => {
    ...
    it('test7: circuit failover routing', async () => {
        ...
        assert.ok(after.status === 'queued' || after.status === 'failed' || after.status === 'deferred', `expected queued or failed or deferred, got ${after.status}`);
    });
    /**
    ...
    it.skip('test12: retry scenario deferred then queued then sent', async () => {
        ...
        await setDoc(doc(db, 'sms_outbox', outboxId), { status: 'queued', retryCount: 1 }, { merge: true });
        await dispatchSmsOutboxCore({
            db,
            outboxId,
...
//# sourceMappingURL=phase-4a-acceptance.test.js.map