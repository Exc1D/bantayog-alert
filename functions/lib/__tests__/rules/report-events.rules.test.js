...
describe('report_events — privileged read with agency scoping', () => {
    ...
    it('suspended superadmin reads report_events fails (negative — isActivePrivileged gate)', async () => {
        const db = authed(env, 'super-suspended', staffClaims({
            role: 'provincial_superadmin',
            permittedMunicipalityIds: ['daet'],
            accountStatus: 'suspended',
        }));
    ...
    it('agency admin reads report_events for own agency (positive)', async () => {
        const db = authed(env, 'bfp-admin', staffClaims({ role: 'agency_admin', agencyId: 'bfp', municipalityId: 'daet' }));
        await assertSucceeds(getDoc(doc(db, 'report_events/re-1')));
    });
    it('agency admin reads report_events for other agency fails (negative)', async () => {
        const db = authed(env, 'redcross-admin', staffClaims({ role: 'agency_admin', agencyId: 'red-cross', municipalityId: 'daet' }));
        await assertFails(getDoc(doc(db, 'report_events/re-1')));
    });
    it('responder reads report_events fails (negative)', async () => {
        const db = authed(env, 'resp-1', staffClaims({ role: 'responder', agencyId: 'bfp', municipalityId: 'daet' }));
        await assertFails(getDoc(doc(db, 'report_events/re-1')));
    });
    ...
    it('any client write to report_events fails', async () => {
        const db = authed(env, 'super-1', staffClaims({ role: 'provincial_superadmin', permittedMunicipalityIds: ['daet'] }));
        await assertFails(setDoc(doc(db, 'report_events/re-new'), {
            agencyId: 'bfp',
            type: 'test',
            municipalityId: 'daet',
            createdAt: ts,
...
describe('dispatch_events — privileged read with agency scoping', () => {
    ...
    it('any client write to dispatch_events fails', async () => {
        const db = authed(env, 'super-1', staffClaims({ role: 'provincial_superadmin', permittedMunicipalityIds: ['daet'] }));
        await assertFails(setDoc(doc(db, 'dispatch_events/de-new'), {
            agencyId: 'bfp',
            type: 'test',
            municipalityId: 'daet',
            createdAt: ts,
...
//# sourceMappingURL=report-events.rules.test.js.map