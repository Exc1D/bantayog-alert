...
describe('report_sharing rules', () => {
    ...
    it('active agency admin whose municipality is shared reads (positive)', async () => {
        const db = authed(env, 'mercedes-agency', staffClaims({ role: 'agency_admin', municipalityId: 'mercedes', agencyId: 'bfp-mercedes' }));
        await assertSucceeds(getDoc(doc(db, 'report_sharing/r-share-1')));
    });
...
describe('report_sharing/events rules', () => {
    it('allows muni admin to write event to subcollection', async () => {
        const db = authed(env, 'daet-admin', staffClaims({ role: 'municipal_admin', municipalityId: 'daet' }));
        await assertSucceeds(addDoc(collection(db, 'report_sharing', 'r-share-1', 'events'), validEvent));
    });
    it('denies a different municipality admin from writing the share event', async () => {
        const db = authed(env, 'mercedes-admin', staffClaims({ role: 'municipal_admin', municipalityId: 'mercedes' }));
        await assertFails(addDoc(collection(db, 'report_sharing', 'r-share-1', 'events'), {
            ...validEvent,
...
describe('report_sharing/events rules — reads', () => {
    ...
    beforeAll(async () => {
        await env.withSecurityRulesDisabled(async (ctx) => {
            await setDoc(doc(ctx.firestore(), 'report_sharing', 'r-share-1', 'events', seededEventId), validEvent);
        });
    });
