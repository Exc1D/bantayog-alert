...
describe('coordination collections rules', () => {
    describe('command_threads', () => {
        ...
        it('command threads are callable-only writes', async () => {
            const db = authed(env, 'daet-admin', staffClaims({ role: 'municipal_admin' }));
            await assertFails(addDoc(collection(db, 'command_threads'), {
                municipalityId: 'daet',
                initiatedBy: 'admin',
                initiatedAt: ts,
                schemaVersion: 1,
    ...
    describe('shift_handoffs', () => {
        ...
        it('shift handoffs are callable-only writes', async () => {
            const db = authed(env, 'daet-admin', staffClaims({ role: 'municipal_admin' }));
            await assertFails(addDoc(collection(db, 'shift_handoffs'), {
                municipalityId: 'daet',
    ...
    describe('mass_alert_requests', () => {
        ...
        it('mass alert requests are callable-only writes', async () => {
            const db = authed(env, 'daet-admin', staffClaims({ role: 'municipal_admin' }));
            await assertFails(addDoc(collection(db, 'mass_alert_requests'), {
                requestedBy: 'admin',
                scope: 'municipality',
                targetIds: ['daet'],
                message: 'Test alert',
    ...
    describe('command_channel_threads (callable)', () => {
        ...
        it('command channel threads are callable-only writes', async () => {
            const db = authed(env, 'daet-admin', staffClaims({ role: 'municipal_admin' }));
            await assertFails(addDoc(collection(db, 'command_channel_threads'), {
                threadId: 'thread-1',
                municipalityId: 'daet',
                createdAt: ts,
            }));
    ...
    describe('command_channel_messages (callable)', () => {
        ...
        it('command channel messages are callable-only writes', async () => {
            const db = authed(env, 'daet-admin', staffClaims({ role: 'municipal_admin' }));
            await assertFails(addDoc(collection(db, 'command_channel_messages'), {
                threadId: 'thread-1',
                message: 'test',
                sentBy: 'admin',
                sentAt: ts,
    ...
    describe('agency_assistance_requests (callable)', () => {
        ...
        it('agency assistance requests are callable-only writes', async () => {
            const db = authed(env, 'daet-admin', staffClaims({ role: 'municipal_admin' }));
            await assertFails(addDoc(collection(db, 'agency_assistance_requests'), {
        ...
        it('agency admin reads request matching their agencyId (positive)', async () => {
            ...
            const db = authed(env, 'bfp-daet-agency', staffClaims({ role: 'agency_admin', municipalityId: 'daet', agencyId: 'bfp-daet' }));
        ...
        it('agency admin denied when agencyId does not match targetAgencyId (negative)', async () => {
            const db = authed(env, 'pnp-daet-agency', staffClaims({ role: 'agency_admin', municipalityId: 'daet', agencyId: 'pnp-daet' }));
    ...
    describe('command_channel_threads/messages participant lookup', () => {
        beforeAll(async () => {
            await env.withSecurityRulesDisabled(async (ctx) => {
                ...
                // Second fixture tests participantUids key lookup path (sentBy vs authorUid)
        ...
        const fixtures = [
            { threadId: 'thread-1', msgId: 'msg-1', description: 'authorUid field' },
            { threadId: 'thread-2', msgId: 'msg-2', description: 'sentBy field' },
        ];
        fixtures.forEach(({ threadId, msgId, description }) => {
            it(`allows participant to read thread (${description})`, async () => {
                const db = authed(env, 'daet-admin', staffClaims({ role: 'municipal_admin', municipalityId: 'daet' }));
                await assertSucceeds(getDoc(doc(db, 'command_channel_threads', threadId)));
            });
            it(`denies non-participant from reading thread (${description})`, async () => {
                const db = authed(env, 'other-admin', staffClaims({ role: 'municipal_admin', municipalityId: 'mercedes' }));
                await assertFails(getDoc(doc(db, 'command_channel_threads', threadId)));
            });
            it(`allows participant to read message (${description})`, async () => {
                const db = authed(env, 'daet-admin', staffClaims({ role: 'municipal_admin', municipalityId: 'daet' }));
                await assertSucceeds(getDoc(doc(db, 'command_channel_messages', msgId)));
            });
            it(`denies non-participant from reading message (${description})`, async () => {
                const db = authed(env, 'other-admin', staffClaims({ role: 'municipal_admin', municipalityId: 'mercedes' }));
                await assertFails(getDoc(doc(db, 'command_channel_messages', msgId)));
            });
        });
    });
});
//# sourceMappingURL=coordination.rules.test.js.map