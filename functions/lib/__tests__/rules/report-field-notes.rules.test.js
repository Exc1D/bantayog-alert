...
describe('reports/field_notes rules', () => {
    ...
    it('allows responder with active dispatch to create a field note', async () => {
        const db = authed(env, 'bfp-responder-1', staffClaims({ role: 'responder', municipalityId: 'daet', agencyId: 'bfp-daet' }));
        await assertSucceeds(addDoc(collection(db, 'reports', 'report-1', 'field_notes'), {
            authorUid: 'bfp-responder-1',
            authorRole: 'responder',
            authorDisplayName: 'BFP Responder 01',
    ...
    it('denies responder without an active dispatch from creating a field note', async () => {
        ...
        await assertFails(addDoc(collection(db, 'reports', 'report-1', 'field_notes'), {
...
//# sourceMappingURL=report-field-notes.rules.test.js.map