...
describe('user_consents rules', () => {
    ...
    it('owner can create consent doc with valid fields', async () => {
        const db = env.authenticatedContext('uid-owner', activeToken).firestore();
        await assertSucceeds(setDoc(doc(db, 'user_consents', 'uid-owner'), {
            consentVersion: '1.0',
            consentGivenAt: Timestamp.now(),
            method: 'in_app_modal',
        }));
    ...
    it('denies create with extra fields', async () => {
        const db = env.authenticatedContext('uid-owner', activeToken).firestore();
        await assertFails(setDoc(doc(db, 'user_consents', 'uid-owner'), {
            consentVersion: '1.0',
            consentGivenAt: Timestamp.now(),
            method: 'in_app_modal',
            extra: 'not-allowed',
    ...
    it('denies create in another user slot', async () => {
        const db = env.authenticatedContext('uid-owner', activeToken).firestore();
        await assertFails(setDoc(doc(db, 'user_consents', 'uid-other'), {
            consentVersion: '1.0',
            consentGivenAt: Timestamp.now(),
            method: 'in_app_modal',
        }));
...
//# sourceMappingURL=user-consents.rules.test.js.map