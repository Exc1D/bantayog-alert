...
describe('SMS layer rules', () => {
    describe('sms_inbox', () => {
        ...
        it('sms inbox is callable-only writes', async () => {
            const db = authed(env, 'citizen-1', staffClaims({ role: 'citizen' }));
            await assertFails(addDoc(collection(db, 'sms_inbox'), {
    ...
    describe('sms_outbox', () => {
        ...
        it('sms outbox is callable-only writes', async () => {
            const db = authed(env, 'citizen-1', staffClaims({ role: 'citizen' }));
            await assertFails(addDoc(collection(db, 'sms_outbox'), {
                toNumber: '+0987654321',
                message: 'test',
                purpose: 'receipt_ack',
                status: 'queued',
    ...
    describe('sms_sessions (callable)', () => {
        ...
        it('sms sessions are callable-only writes', async () => {
            const db = authed(env, 'citizen-1', staffClaims({ role: 'citizen' }));
            await assertFails(addDoc(collection(db, 'sms_sessions'), {
                provider: 'semaphore',
                sessionKey: 'test',
                expiresAt: ts,
            }));
    ...
    describe('sms_provider_health (callable)', () => {
        ...
        it('sms provider health are callable-only writes', async () => {
            const db = authed(env, 'citizen-1', staffClaims({ role: 'citizen' }));
            await assertFails(addDoc(collection(db, 'sms_provider_health'), {
                provider: 'semaphore',
                isHealthy: true,
                checkedAt: ts,
            }));
...
//# sourceMappingURL=sms.rules.test.js.map