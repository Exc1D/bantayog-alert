...
describe('hazard zones rules', () => {
    describe('hazard_zones', () => {
        ...
        it('hazard zone writes are callable-only', async () => {
            const db = authed(env, 'daet-admin', staffClaims({ role: 'municipal_admin', municipalityId: 'daet' }));
            await assertFails(setDoc(doc(db, 'hazard_zones/zone-1'), {
                zoneId: 'zone-1',
                version: 1,
                hazardType: 'flood',
                scope: 'municipality',
    ...
    describe('hazard_signals', () => {
        ...
        it('hazard signals are callable-only writes', async () => {
            const db = authed(env, 'daet-admin', staffClaims({ role: 'municipal_admin', municipalityId: 'daet' }));
            await assertFails(addDoc(collection(db, 'hazard_signals'), {
                zoneId: 'zone-1',
                version: 1,
                detectedAt: ts,
                severity: 'high',
    ...
    describe('hazard_zones_history', () => {
        ...
        it('hazard zones history are callable-only writes', async () => {
            const db = authed(env, 'daet-admin', staffClaims({ role: 'municipal_admin', municipalityId: 'daet' }));
            await assertFails(addDoc(collection(db, 'hazard_zones_history'), {
                zoneId: 'zone-1',
                version: 2,
                previousVersion: 1,
                replacedBy: 'admin',
...
//# sourceMappingURL=hazard-zones.rules.test.js.map