...
function mockFetch(data, ok = true, status = 200) {
    ...
    return vi.spyOn(global, 'fetch').mockResolvedValue(res);
}
...
describe('createGlobelabsSmsProvider', () => {
    it('sends SMS successfully with cached token', async () => {
        ...
        const provider = createGlobelabsSmsProvider({ getFirestore: () => mockDb });
    ...
    it('throws SmsProviderRetryableError on 429 rate limit', async () => {
        ...
        mockFetch({}, false, 429);
        const provider = createGlobelabsSmsProvider({ getFirestore: () => mockDb });
        await expect(provider.send({ to: '+639171234567', body: 'Hello', encoding: 'GSM-7' })).rejects.toThrow('globelabs 429');
    });
    it('throws SmsProviderRetryableError on 500 server error', async () => {
        ...
        mockFetch({}, false, 500);
        const provider = createGlobelabsSmsProvider({ getFirestore: () => mockDb });
        await expect(provider.send({ to: '+639171234567', body: 'Hello', encoding: 'GSM-7' })).rejects.toThrow('globelabs 500');
    });
        ...
        it('only refreshes token once when multiple callers need it', async () => {
            ...
            vi.spyOn(global, 'fetch').mockImplementation((url) => {
                ...
                return Promise.resolve({
                    ok: true,
                    status: 200,
                    json: () => Promise.resolve({ outboundSMSMessageRequest: { resourceURL: 'msg/mutex-test' } }),
                });
            });
            const provider = createGlobelabsSmsProvider({ getFirestore: () => mockDb });
...
//# sourceMappingURL=globelabs.test.js.map