...
function baseOutbox(id, overrides = {}) {
    return {
...
describe('reconcileSmsDeliveryStatusCore', () => {
    ...
    it('CAS deferred → queued and updates queuedAt to now', async () => {
        ...
        await db
            .collection('sms_outbox')
            .doc('def-1')
            .set(baseOutbox('def-1', { status: 'deferred', retryCount: 1, queuedAt: now - 10 * 60 * 1000 }));
        await reconcileSmsDeliveryStatusCore({ db, now: () => now });
...
//# sourceMappingURL=reconcile-sms-delivery-status.integration.test.js.map