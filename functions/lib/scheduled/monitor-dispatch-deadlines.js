...
function markDispatchNeedsAdmin(tx, dispatchRef, dispatchId, now, monitorRunId) {
...
function writeDeadlineExceededEvent(tx, db, dispatchId, assignedTo, escalationCount, now) {
...
function writeEscalationAttemptedEvent(tx, db, dispatchId, fromResponder, toResponder, escalationCount, now) {
    tx.set(db.collection('dispatch_events').doc(), {
...
async function queryAvailableResponders(db, municipalityIds, now, windowMs) {
    const chunks = [];
    ...
    const snapPromises = chunks.map((chunk) => db
...
function findCandidateResponder(allResponders, dispatchData) {
...
async function updateNeedsAdminAlerts(municipalityIds, now, count) {
...
export async function monitorDispatchDeadlinesCore(db, deps) {
    ...
    const municipalityIds = [
        ...new Set(dispatchDocs.map((d) => d.data().municipalityId)),
    ];
    ...
    if (allResponders.length > RESPONDER_CAP) {
        log({
            ...
            message: 'Capped responders from ' + String(allResponders.length) + ' to ' + String(RESPONDER_CAP),
        });
        ...
        try {
            await db.runTransaction(async (tx) => {
                ...
                if ((data.escalationCount ?? 0) >= 1) {
                    ...
                    writeDeadlineExceededEvent(tx, db, dispatchDoc.id, assignedTo, data.escalationCount ?? 0, now);
                    needsAdminCount++;
                    return;
                }
                const responderSnap = await tx.get(db.collection('responders').doc(assignedTo.uid));
                if (!responderSnap.exists ||
                    responderSnap.data().accountStatus !== 'active') {
                    markDispatchNeedsAdmin(tx, dispatchRef, dispatchDoc.id, now, monitorRunId);
                    needsAdminCount++;
                    return;
                }
                ...
                writeEscalationAttemptedEvent(tx, db, dispatchDoc.id, assignedTo, candidate, newEscalationCount, now);
                escalatedCount++;
            });
        }
        catch (err) {
...
//# sourceMappingURL=monitor-dispatch-deadlines.js.map