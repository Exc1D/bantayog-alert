...
describe('processInboxItemCore', () => {
    itif(available)('materializes a complete triptych + event + lookup from a valid inbox doc', async () => {
        await env.withSecurityRulesDisabled(async (ctx) => {
    ...
    itif(available)('writes 6-char locationGeohash onto report_ops when exactLocation present', async () => {
        await env.withSecurityRulesDisabled(async (ctx) => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const db = ctx.firestore();
    ...
    itif(available)('omits locationGeohash from report_ops when exactLocation is absent', async () => {
        await env.withSecurityRulesDisabled(async (ctx) => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const db = ctx.firestore();
    ...
    itif(available)('writes moderation_incident and throws when payload schema is invalid', async () => {
        await env.withSecurityRulesDisabled(async (ctx) => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const db = ctx.firestore();
    ...
    itif(available)('writes moderation_incident and throws when location is out of jurisdiction', async () => {
        await env.withSecurityRulesDisabled(async (ctx) => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const db = ctx.firestore();
    ...
    itif(available)('writes moderation_incident with reason location_missing when publicLocation is absent', async () => {
        await env.withSecurityRulesDisabled(async (ctx) => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const db = ctx.firestore();
            ...
            await expect(processInboxItemCore({ db, inboxId: 'ibx-nopublic', now: () => 1713350401000 })).rejects.toMatchObject({ code: 'INVALID_ARGUMENT' });
            const incidentSnap = await getDoc(doc(ctx.firestore(), 'moderation_incidents', 'ibx-nopublic'));
            expect(incidentSnap.exists()).toBe(true);
            expect(incidentSnap.data()?.reason).toBe('location_missing');
        });
    });
    itif(available)('materializes report without media when pendingMediaIds references a missing doc', async () => {
        await env.withSecurityRulesDisabled(async (ctx) => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const db = ctx.firestore();
            ...
            expect(result.materialized).toBe(true);
            const mediaDocs = await getDocs(collection(ctx.firestore(), 'reports', result.reportId, 'media'));
