...
import { requestLookupImpl } from '../request-lookup.js';
const mockGet = vi.fn();
function db() {
    return {
...
describe('requestLookupImpl — both-codes path', () => {
    ...
    it('returns NOT_FOUND when the public ref does not exist', async () => {
        ...
        await expect(requestLookupImpl({ db: db(), data: { publicRef: 'a1b2c3d4', secret } })).rejects.toMatchObject({ code: 'NOT_FOUND' });
    });
    it('returns FORBIDDEN on secret mismatch', async () => {
        ...
        await expect(requestLookupImpl({ db: db(), data: { publicRef: 'a1b2c3d4', secret: 'wrong' } })).rejects.toMatchObject({ code: 'FORBIDDEN' });
    });
...
describe('requestLookupImpl — secret-only path', () => {
    ...
    it('returns UNAUTHORIZED when auth is absent', async () => {
        await expect(requestLookupImpl({ db: db(), data: { secret }, auth: undefined })).rejects.toMatchObject({ code: 'UNAUTHORIZED' });
    });
    it('returns NOT_FOUND when secret_lookup doc does not exist', async () => {
        mockGet.mockResolvedValue({ exists: false });
        await expect(requestLookupImpl({ db: db(), data: { secret }, auth: { uid: 'u1' } })).rejects.toMatchObject({ code: 'NOT_FOUND' });
    });
...
//# sourceMappingURL=request-lookup.test.js.map