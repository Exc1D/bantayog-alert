...
beforeEach(() => {
    ...
    mockIsAccountActive.mockImplementation((claims) => claims.accountStatus === 'active');
});
afterEach(() => {
    vi.restoreAllMocks();
});
...
const handler = submitReportFeedback;
describe('submitReportFeedback callable wrapper', () => {
    ...
    it('rejects callers with missing auth token', async () => {
        await expect(handler(createRequest({ auth: { uid: 'citizen-1', token: null } }))).rejects.toMatchObject({
            code: 'unauthenticated',
            message: 'sign-in required',
        });
    });
    it('rejects non-citizen roles', async () => {
        await expect(handler(createRequest({
    ...
    it('rejects malformed payload (invalid addressed type)', async () => {
        await expect(handler(createRequest({ data: { reportId: 'report-1', addressed: 'yes' } }))).rejects.toMatchObject({
            code: 'invalid-argument',
            message: 'malformed payload',
        });
    });
...
//# sourceMappingURL=submit-report-feedback.callable.test.js.map