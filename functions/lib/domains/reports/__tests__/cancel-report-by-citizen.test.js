...
describe('cancelReportByCitizenCore', () => {
    ...
    itif(available)('withdraws report when status is awaiting_verify and citizen owns it', async () => {
        await testEnv.withSecurityRulesDisabled(async (ctx) => {
            const db = ctx.firestore();
    ...
    itif(available)('rejects non-existent report with NOT_FOUND', async () => {
        await testEnv.withSecurityRulesDisabled(async (ctx) => {
            ...
            await expect(cancelReportByCitizenCore(db, {
    ...
    itif(available)('is idempotent — replay with same key succeeds without error', async () => {
        await testEnv.withSecurityRulesDisabled(async (ctx) => {
            ...
            const events = await db.collection('report_events').where('reportId', '==', reportId).get();
            const cancelEvents = events.docs.filter((doc) => doc.data().to === 'citizen_cancelled');
