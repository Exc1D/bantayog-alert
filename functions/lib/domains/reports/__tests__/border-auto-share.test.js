...
import { borderAutoShareCore } from '../border-auto-share.js';
let testEnv;
let available = false;
beforeAll(async () => {
...
describe('borderAutoShareTrigger', () => {
    itif(available)('skips reports with no locationGeohash', async () => {
        ...
        await borderAutoShareCore(adminDb, {
            reportId: 'r1',
            opsData: opsDoc,
            boundaryGeohashSet: new Set([NEAR_BOUNDARY_GEOHASH]),
        });
    ...
    itif(available)('does not re-trigger if report already shared with that municipality', async () => {
        const opsDoc = {
            municipalityId: 'daet',
            locationGeohash: NEAR_BOUNDARY_GEOHASH,
            status: 'verified',
...
//# sourceMappingURL=border-auto-share.test.js.map