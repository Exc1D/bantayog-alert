...
describe('dispatchMirrorToReport', () => {
    ...
    itif(available)('reverts declined dispatches back to verified and clears currentDispatchId', async () => {
        const { reportId, dispatchId } = await seedAcceptedDispatch();
        await withAdminDb(async (db) => {
            await dispatchMirrorToReportCore({
                db,
    ...
    itif(available)('reverts timed out dispatches back to verified and clears currentDispatchId', async () => {
        const { reportId, dispatchId } = await seedAcceptedDispatch();
        await withAdminDb(async (db) => {
            await dispatchMirrorToReportCore({
                db,
