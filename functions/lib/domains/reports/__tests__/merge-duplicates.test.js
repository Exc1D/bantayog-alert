...
import { mergeDuplicatesCore } from '../merge-duplicates.js';
...
describe('mergeDuplicates', () => {
    ...
    emulatorIt('aggregates unique mediaRefs from duplicates onto the primary', async () => {
        ...
        const refs = primary.data()?.mediaRefs;
    ...
    emulatorIt('is idempotent', async () => {
        ...
        const result2 = await mergeDuplicatesCore(adminDb, {
            primaryReportId: 'r-primary',
            duplicateReportIds: ['r-dup1'],
            idempotencyKey: uuid(7),
        }, muniAdminActor);
    ...
    emulatorIt('updates report_ops for primary and duplicates', async () => {
        ...
        expect((primaryOps.data()?.updatedAt).toMillis()).toBeGreaterThan(ts);
        expect((dupOps.data()?.updatedAt).toMillis()).toBeGreaterThan(ts);
    });
});
//# sourceMappingURL=merge-duplicates.test.js.map