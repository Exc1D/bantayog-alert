...
import { unpublishReportCore } from '../unpublish-report.js';
import { seedActiveAccount, seedReportAtStatus, staffClaims, } from '../../../__tests__/helpers/seed-factories.js';
let testEnv;
let available = false;
beforeAll(async () => {
...
describe('unpublishReportCore', () => {
    ...
    it('rejects cross-municipality takedowns by municipal admins', async ({ skip }) => {
        ...
        await testEnv.withSecurityRulesDisabled(async (ctx) => {
            ...
            await expect(unpublishReportCore(db, {
                reportId,
                reason: 'sensitive_content',
                idempotencyKey: crypto.randomUUID(),
                actor: {
...
//# sourceMappingURL=unpublish-report.test.js.map