...
function isPlainAnonymousUser(user) {
    return (user.providerData.length === 0 &&
        user.email === undefined &&
        user.phoneNumber === undefined &&
        !hasCustomClaims(user));
}
function lastAuthActivityMs(user) {
    return (parseAuthTimestamp(user.metadata.lastSignInTime) ??
        parseAuthTimestamp(user.metadata.creationTime));
}
export async function anonymousAuthCleanupCore(input) {
...
//# sourceMappingURL=anonymous-auth-cleanup.js.map