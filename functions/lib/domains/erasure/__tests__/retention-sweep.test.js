...
import { retentionSweepCore } from '../retention-sweep.js';
...
describe('retentionSweepCore', () => {
    itif(available)('anonymizes unverified report older than 1 week and sets retentionHardDeleteEligibleAt', async () => {
        await env.withSecurityRulesDisabled(async (ctx) => {
            const db = ctx.firestore();
    ...
    itif(available)('hard-deletes report when retentionHardDeleteEligibleAt is in the past', async () => {
        await env.withSecurityRulesDisabled(async (ctx) => {
            const db = ctx.firestore();
            const { getStorage } = await import('firebase-admin/storage');
...
//# sourceMappingURL=retention-sweep.test.js.map