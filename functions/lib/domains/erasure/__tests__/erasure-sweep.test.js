...
describe('erasureSweepCore', () => {
    itif(available)('anonymizes report fields, deletes storage, and deletes Auth on approved request', async () => {
        await env.withSecurityRulesDisabled(async (ctx) => {
            ...
            mockGetFiles.mockResolvedValueOnce([
                [
                    {
                        delete: () => mockDeleteFile('report_media/report-1/photo1.jpg'),
                    },
                    {
                        delete: () => mockDeleteFile('report_media/report-1/photo2.jpg'),
                    },
                ],
            ]);
    ...
    itif(available)('dead-letters and re-enables Auth on erasure failure when re-enable succeeds', async () => {
        await env.withSecurityRulesDisabled(async (ctx) => {
            const db = ctx.firestore();
            const { getAuth } = await import('firebase-admin/auth');
            const { getStorage } = await import('firebase-admin/storage');
    ...
    itif(available)('throws and dead-letters when Auth re-enable fails after erasure failure', async () => {
        await env.withSecurityRulesDisabled(async (ctx) => {
            const db = ctx.firestore();
            const { getAuth } = await import('firebase-admin/auth');
            const { getStorage } = await import('firebase-admin/storage');
            ...
            await expect(erasureSweepCore({ db, auth: getAuth(), storage: getStorage() })).rejects.toThrow('re-enable failed');
    ...
    itif(available)('throws claim_lost_race when record is no longer eligible', async () => {
        await env.withSecurityRulesDisabled(async (ctx) => {
            ...
            await expect(erasureSweepCore({ db, auth: getAuth(), storage: getStorage(), now: mockNow })).rejects.toThrow('claim_lost_race');
        });
    });
