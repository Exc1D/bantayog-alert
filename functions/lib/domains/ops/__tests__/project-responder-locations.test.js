...
import { projectResponderLocationsCore, roundToGrid, computeFreshness, } from '../project-responder-locations.js';
...
describe('projectResponderLocationsCore', () => {
    itif(available)('projects active responders with rounded coordinates and correct freshness', async () => {
    ...
    itif(available)('groups responders by municipalityId', async () => {
        await testEnv.withSecurityRulesDisabled(async (ctx) => {
            ...
            await projectResponderLocationsCore(rtdb, { now });
            const daet = (await rtdb.ref('shared_projection/daet/r1').get()).val();
            const mercedes = (await rtdb.ref('shared_projection/mercedes/r2').get()).val();
            expect(daet?.freshness).toBe('fresh');
            expect(mercedes?.freshness).toBe('fresh');
        });
    });
});
//# sourceMappingURL=project-responder-locations.test.js.map