...
import { initiateResponderHandoffCore, acceptResponderHandoffCore, } from '../responder-shift-handoff.js';
...
function makeActor(uid, overrides = {}) {
    return {
        uid,
        claims: {
            role: 'responder',
...
async function seedResponder(responderId, overrides = {}) {
...
async function createHandoff(id, overrides = {}) {
    await testEnv.withSecurityRulesDisabled(async (ctx) => {
...
describe('initiateResponderHandoffCore', () => {
    itif(available)('should reject when actor account status is not active', async () => {
        const result = await initiateResponderHandoffCore(adminDb, { toUid: 'responder-b', reason: 'shift ended', idempotencyKey: uuid(1) }, makeActor('responder-a', { accountStatus: 'inactive' }), 'corr-1');
    ...
    itif(available)('should reject when toUid equals actor uid', async () => {
        const result = await initiateResponderHandoffCore(adminDb, { toUid: 'responder-a', reason: 'shift ended', idempotencyKey: uuid(2) }, makeActor('responder-a'), 'corr-2');
        expect(result.success).toBe(false);
        if (!result.success) {
    ...
    itif(available)('should reject when toUid responder is not found', async () => {
        ...
        const result = await initiateResponderHandoffCore(adminDb, { toUid: 'responder-b', reason: 'shift ended', idempotencyKey: uuid(3) }, makeActor('responder-a'), 'corr-3');
        expect(result.success).toBe(false);
        if (!result.success) {
    ...
    itif(available)('should create handoff document when valid', async () => {
        ...
        const result = await initiateResponderHandoffCore(adminDb, { toUid: 'responder-b', reason: 'shift ended', idempotencyKey: uuid(4) }, makeActor('responder-a'), 'corr-4');
    ...
    itif(available)('should be idempotent when called twice with the same idempotencyKey', async () => {
        await seedResponder('responder-a');
        await seedResponder('responder-b');
        const input = { toUid: 'responder-b', reason: 'shift ended', idempotencyKey: uuid(5) };
        const result1 = await initiateResponderHandoffCore(adminDb, input, makeActor('responder-a'), 'corr-5a');
        const result2 = await initiateResponderHandoffCore(adminDb, input, makeActor('responder-a'), 'corr-5b');
...
describe('acceptResponderHandoffCore', () => {
    itif(available)('should reject when handoff is not found', async () => {
        const result = await acceptResponderHandoffCore(adminDb, { handoffId: 'missing-handoff', idempotencyKey: uuid(6) }, makeActor('responder-b'), 'corr-6');
        expect(result.success).toBe(false);
        if (!result.success) {
            expect(result.errorCode).toBe('not-found');
        }
    ...
    itif(available)('should reject when accepting uid is not the toUid', async () => {
        ...
        const result = await acceptResponderHandoffCore(adminDb, { handoffId: 'handoff-1', idempotencyKey: uuid(7) }, makeActor('responder-b'), 'corr-7');
        expect(result.success).toBe(false);
        if (!result.success) {
            expect(result.errorCode).toBe('permission-denied');
        }
    ...
    itif(available)('should mark handoff as accepted', async () => {
        ...
        const result = await acceptResponderHandoffCore(adminDb, { handoffId: 'handoff-2', idempotencyKey: uuid(8) }, makeActor('responder-b'), 'corr-8');
        expect(result.success).toBe(true);
...
//# sourceMappingURL=responder-shift-handoff.test.js.map