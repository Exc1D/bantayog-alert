...
import { requestAgencyAssistanceCore, acceptAgencyAssistanceCore, declineAgencyAssistanceCore, } from '../callables.js';
...
describe('requestAgencyAssistance', () => {
    ...
    itif(available)('rejects a muni admin requesting for a report in another municipality', async () => {
    ...
    itif(available)('creates a command_channel_thread with threadType agency_assistance', async () => {
        await seedReport('r1');
        await seedActiveAccount(testEnv, { uid: 'bfp-admin', role: 'agency_admin', agencyId: 'bfp' });
        const result = await requestAgencyAssistanceCore(adminDb, {
            reportId: 'r1',
    ...
    itif(available)('is idempotent — double-call returns success without duplicate docs', async () => {
        await seedReport('r1');
        await seedActiveAccount(testEnv, { uid: 'bfp-admin', role: 'agency_admin', agencyId: 'bfp' });
