...
import { initiateShiftHandoffCore, acceptShiftHandoffCore } from '../shift-handoff.js';
...
async function seedReportOp(id) {
...
describe('initiateShiftHandoff', () => {
    itif(available)('rejects citizens and responders', async () => {
        const result = await initiateShiftHandoffCore(adminDb, {
    ...
    itif(available)('builds activeIncidentSnapshot from live Firestore state', async () => {
        ...
        const created = await adminDb
            ...
            .doc(result.success ? result.handoffId : '')
            .get();
        const snapshot = created.data()?.activeIncidentIds;
        expect(snapshot).toContain('r-active-1');
        expect(snapshot).toContain('r-active-2');
    });
    itif(available)('is idempotent', async () => {
        const result1 = await initiateShiftHandoffCore(adminDb, {
            notes: '',
            idempotencyKey: uuid(4),
        }, adminActor, 'corr-4');
        expect(result1.success).toBe(true);
        const result2 = await initiateShiftHandoffCore(adminDb, {
            notes: '',
            idempotencyKey: uuid(4),
        }, adminActor, 'corr-5');
...
describe('acceptShiftHandoff', () => {
    ...
    itif(available)('rejects inactive admin', async () => {
        ...
        const result = await acceptShiftHandoffCore(adminDb, { handoffId: 'h-inactive', idempotencyKey: uuid(12) }, {
            uid: 'admin-to',
            claims: {
                role: 'municipal_admin',
                municipalityId: 'daet',
    ...
    itif(available)('rejects non-existent handoff', async () => {
        const result = await acceptShiftHandoffCore(adminDb, { handoffId: 'h-missing', idempotencyKey: uuid(13) }, {
            uid: 'admin-to',
            claims: {
                role: 'municipal_admin',
                municipalityId: 'daet',
    ...
    itif(available)('rejects expired handoff', async () => {
        ...
        const result = await acceptShiftHandoffCore(adminDb, { handoffId: 'h-expired', idempotencyKey: uuid(14) }, {
            uid: 'admin-to',
            claims: {
                role: 'municipal_admin',
                municipalityId: 'daet',
    ...
    itif(available)('rejects self-accept', async () => {
        ...
        const result = await acceptShiftHandoffCore(adminDb, { handoffId: 'h-self', idempotencyKey: uuid(15) }, {
            uid: 'admin-from',
            claims: {
                role: 'municipal_admin',
                municipalityId: 'daet',
    ...
    itif(available)('rejects a caller from a different municipality', async () => {
        ...
        const result = await acceptShiftHandoffCore(adminDb, { handoffId: 'h1', idempotencyKey: uuid(5) }, {
            uid: 'other-admin',
            claims: {
                role: 'municipal_admin',
                municipalityId: 'labo',
    ...
    itif(available)('updates status to accepted and sets toUid', async () => {
        ...
        const result = await acceptShiftHandoffCore(adminDb, { handoffId: 'h2', idempotencyKey: uuid(6) }, {
            uid: 'admin-to',
            claims: {
                role: 'municipal_admin',
                municipalityId: 'daet',
    ...
    itif(available)('is idempotent — double-accept returns success', async () => {
        ...
        const result1 = await acceptShiftHandoffCore(adminDb, { handoffId: 'h3', idempotencyKey: uuid(7) }, actor, 'corr-8');
        expect(result1.success).toBe(true);
