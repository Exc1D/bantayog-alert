...
async function seedResponder(db, uid, muni, status) {
...
async function seedDispatchNeedsAdmin(db, dispatchId, opts) {
    await db
...
describe('escalateDispatchCore', () => {
    ...
    itif(available)('rejects municipal_admin escalating dispatch in other municipality', async () => {
        await testEnv.withSecurityRulesDisabled(async (ctx) => {
            ...
            await expect(escalateDispatchCore(db, {
                dispatchId: 'd1',
                newResponderUid: 'responder-2',
                idempotencyKey: crypto.randomUUID(),
                actor: {
...
//# sourceMappingURL=escalate-dispatch.test.js.map