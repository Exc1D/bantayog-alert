...
describe('dispatchResponderCore', () => {
    itif(available)('creates dispatch, transitions report → assigned, writes both event streams', async () => {
        await testEnv.withSecurityRulesDisabled(async (ctx) => {
            ...
            const reportEvents = await db
                ...
                .get();
            const reportEventData = reportEvents.docs.map((doc) => doc.data());
            expect(reportEventData.filter((event) => event.from === 'verified')).toHaveLength(1);
            const reportNotificationEvents = reportEventData.filter((event) => event.type === 'notification_attempted');
...
describe('dispatchResponderCore error paths', () => {
    itif(available)('PERMISSION_DENIED when responder is in another municipality', async () => {
        await testEnv.withSecurityRulesDisabled(async (ctx) => {
            ...
            await expect(dispatchResponderCore(db, rtdb, {
                reportId,
...
//# sourceMappingURL=dispatch-responder.test.js.map