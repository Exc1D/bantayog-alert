...
describe('redispatch policy', () => {
    it('accepts terminal dispatch states and rejects non-terminal states', () => {
        ...
        try {
            ...
            expect(err.code).toBe(BantayogErrorCode.FAILED_PRECONDITION);
            expect(err.message).toContain('Cannot redispatch from status pending');
        }
    });
    it('derives actor municipality ids from both claim eras', () => {
        expect(getActorMunicipalityIds({
            municipalityId: 'daet',
...
//# sourceMappingURL=redispatch-policy.test.js.map