...
describe('declineDispatchCore', () => {
    itif(available)('declines a pending dispatch with a required reason', async () => {
    ...
    itif(available)('rejects when declineReason is blank', async () => {
        ...
        await testEnv.withSecurityRulesDisabled(async (ctx) => {
            const db = ctx.firestore();
            await expect(declineDispatchCore(db, {
    ...
    itif(available)('rejects when declineReason exceeds 200 characters', async () => {
        ...
        const callDeclineDispatch = declineDispatch;
        await expect(callDeclineDispatch({
            auth: {
                uid: 'r1',
    ...
    itif(available)('rejects when dispatch is not pending', async () => {
    ...
    itif(available)('rejects when the dispatch is assigned to another responder', async () => {
    ...
    itif(available)('rejects when dispatch.assignedTo is missing', async () => {
    ...
    itif(available)('rejects when dispatch.assignedTo.uid matches but agencyId is missing', async () => {
    ...
    itif(available)('returns the same result without duplicating events when replayed with the same idempotency key', async () => {
...
describe('declineDispatch callable', () => {
    const callDeclineDispatch = declineDispatch;
    itif(available)('wires App Check config and accepts an authenticated responder request', async () => {
    ...
    itif(available)('rejects an unauthenticated request', async () => {
        await expect(callDeclineDispatch({
            data: {
    ...
    itif(available)('rejects a wrong-role request', async () => {
        await expect(callDeclineDispatch({
            auth: {
                uid: 'admin-1',
    ...
    itif(available)('surfaces not-found when dispatch is missing', async () => {
        await seedActiveAccount(testEnv, {
            uid: 'r1',
            role: 'responder',
            municipalityId: 'daet',
    ...
    itif(available)('surfaces permission-denied when dispatch.assignedTo is missing', async () => {
    ...
    itif(available)('surfaces permission-denied when dispatch.assignedTo.uid matches but municipalityId is missing', async () => {
    ...
    itif(available)('surfaces resource-exhausted when responder exceeds 30 declines per minute', async () => {
        await seedActiveAccount(testEnv, {
        ...
        for (let i = 0; i < 31; i++) {
            ...
            await seedReportAtStatusJS(testEnv, reportId, 'assigned');
            await seedDispatchJS(testEnv, dispatchId, reportId, 'responder-callable-rate-limit', 'pending');
        }
        for (let i = 0; i < 30; i++) {
    ...
    itif(available)('rejects idempotency key replay with different payload', async () => {
        await seedReportAtStatusJS(testEnv, 'report-idempotency-mismatch', 'assigned');
        await seedDispatchJS(testEnv, 'dispatch-idempotency-mismatch', 'report-idempotency-mismatch', 'r1', 'pending');
        await seedActiveAccount(testEnv, {
            uid: 'r1',
            role: 'responder',
            municipalityId: 'daet',
...
//# sourceMappingURL=decline-dispatch.test.js.map