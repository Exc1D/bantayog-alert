...
import { seedActiveAccount, seedDispatch, seedReportAtStatus, } from '../../../__tests__/helpers/seed-factories.js';
let testEnv;
let available = false;
beforeAll(async () => {
...
describe('advanceDispatchCore', () => {
    ...
    itif(available)('rejects INVALID_STATUS_TRANSITION for backward steps (en_route -> acknowledged)', async () => {
        await testEnv.withSecurityRulesDisabled(async (ctx) => {
            const db = ctx.firestore();
