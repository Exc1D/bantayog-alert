...
describe('cancelDispatchCore (3b branches)', () => {
    ...
    itif(available)('PERMISSION_DENIED when cancelling a dispatch for a different muni', async () => {
        await testEnv.withSecurityRulesDisabled(async (ctx) => {
            ...
            await expect(cancelDispatchCore(db, {
                dispatchId,
                reason: 'responder_unavailable',
                idempotencyKey: crypto.randomUUID(),
                actor: {
    ...
    itif(available)('cancels non-current dispatch without reverting report status', async () => {
        await testEnv.withSecurityRulesDisabled(async (ctx) => {
            ...
            await db
                .collection('reports')
                .doc(reportId)
                .update({ currentDispatchId: 'newer-dispatch-id' });
            const { dispatchId } = await seedDispatch(db, {
                reportId,
                responderUid: 'r1',
                municipalityId: 'daet',
    ...
    itif(available)('FAILED_PRECONDITION when dispatch is already cancelled (double-cancel)', async () => {
        await testEnv.withSecurityRulesDisabled(async (ctx) => {
            const db = ctx.firestore();
            const { reportId } = await seedReportAtStatus(db, 'assigned', { municipalityId: 'daet' });
            const { dispatchId } = await seedDispatch(db, {
...
describe('cancelDispatch — widened from-state (3c)', () => {
    const CANCELLABLE_FROM = ['accepted', 'acknowledged', 'en_route', 'on_scene'];
    for (const from of CANCELLABLE_FROM) {
        itif(available)(`allows cancel from ${from} → status=cancelled, report reverted to verified`, async () => {
            await testEnv.withSecurityRulesDisabled(async (ctx) => {
                const db = ctx.firestore();
                const { reportId } = await seedReportAtStatus(db, 'assigned', { municipalityId: 'daet' });
                const { dispatchId } = await seedDispatch(db, {
...
//# sourceMappingURL=cancel-dispatch.test.js.map