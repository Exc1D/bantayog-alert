...
export function projectHazardSignalStatus(input) {
    ...
    for (const { id: municipalityId } of CAMARINES_NORTE_MUNICIPALITIES) {
        ...
        effectiveScopes.push({
            municipalityId,
            signalLevel: winner.signalLevel,
    ...
    return {
        ...
        scopeType: coveredCount === allMunicipalities ? 'province' : active ? 'municipalities' : undefined,
        affectedMunicipalityIds: effectiveScopes.map((s) => s.municipalityId),
        effectiveScopes,
        validUntil: active
            ? Math.min(...effectiveScopes.map((scope) => {
...
export async function replayHazardSignalProjection(input) {
    ...
    const currentStatus = currentStatusSnap.exists
        ? currentStatusSnap.data()
        : undefined;
    const invalidSignalIds = [];
    const signals = snap.docs.flatMap((d) => {
    ...
    const status = projectHazardSignalStatus({
        now: input.now,
        signals,
...
//# sourceMappingURL=hazard-signal-projector.js.map