...
const TODAY_COST_SQL = 'SELECT IFNULL(SUM(cost), 0) AS totalCost FROM `cloud_billing_export.gcp_billing_export_v1_*` WHERE DATE(usage_start_time) = CURRENT_DATE()';
const BASELINE_COST_SQL = 'SELECT IFNULL(AVG(dailyCost), 0) AS totalCost FROM (SELECT SUM(cost) AS dailyCost FROM `cloud_billing_export.gcp_billing_export_v1_*` WHERE DATE(usage_start_time) BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY) AND DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY) GROUP BY DATE(usage_start_time))';
function extractCost(rows) {
    const row = rows[0];
 ...
 */
export async function costSnapshotWriterCore(db, bigQuery, deps = {}) {
...
export const costSnapshotWriter = onSchedule({ schedule: '15 0 * * *', region: 'asia-southeast1', timeoutSeconds: 300, timeZone: 'UTC' }, async () => {
    await costSnapshotWriterCore(getFirestore(), bq);
});
//# sourceMappingURL=cost-snapshot-writer.js.map