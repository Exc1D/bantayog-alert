...
export const onMediaRelocate = onObjectFinalized({ region: 'asia-southeast1', minInstances: 0, maxInstances: 20, timeoutSeconds: 60 }, async (event) => {
    const flagSnap = await getFirestore().collection('system_config').doc('features').get();
    const enabled = flagSnap.exists
        ? Boolean(flagSnap.data()
...
//# sourceMappingURL=on-media-relocate.js.map