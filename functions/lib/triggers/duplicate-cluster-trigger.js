...
const BATCH_CAP = 250;
export async function duplicateClusterTriggerCore(db, snap) {
    const data = snap.data();
    const { locationGeohash, municipalityId, reportType, createdAt, duplicateClusterId: existingCluster, } = data;
    if (typeof locationGeohash !== 'string' || locationGeohash.length < 6)
        return;
    ...
    const nearby = candidates.docs.filter((d) => {
        ...
        if (typeof gh !== 'string' || gh.length < 6)
            return false;
        if (!neighborPrefixes.has(gh.slice(0, 6)))
            return false;
        try {
    ...
    const existingClusterFromNearby = nearby
        .map((d) => d.data().duplicateClusterId)
    ...
    if (toUpdate.length === 0 && existingCluster === clusterId)
        return;
...
export const duplicateClusterTrigger = onDocumentCreated({ document: 'report_ops/{reportId}', region: 'asia-southeast1' }, async (event) => {
...
//# sourceMappingURL=duplicate-cluster-trigger.js.map