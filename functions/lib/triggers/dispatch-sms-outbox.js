...
import { pickProvider, incrementMinuteWindow, NoProviderAvailableError, } from '../services/sms-health.js';
import { SmsProviderRetryableError } from '../services/sms-provider.js';
...
export async function dispatchSmsOutboxCore(args) {
    ...
    const outboxData = (await outboxRef.get()).data();
    // Pick provider.
    let providerTarget;
    try {
    ...
    try {
        ...
        await incrementMinuteWindow(db, providerTarget, { success: false, rateLimited: isRate, latencyMs }, now());
        log({
            severity: 'WARNING',
    ...
    if (result.accepted) {
        ...
        await incrementMinuteWindow(db, providerTarget, { success: true, rateLimited: false, latencyMs }, now());
        log({
            severity: 'INFO',
            code: 'sms.sent',
            message: outboxId,
        ...
        await outboxRef.update({
            status: 'failed',
            failedAt: now(),
            terminalReason: result.reason === 'invalid_number' || result.reason === 'bad_format'
                ? 'rejected'
        ...
        await incrementMinuteWindow(db, providerTarget, { success: false, rateLimited: false, latencyMs }, now());
        log({
            severity: 'INFO',
            code: 'sms.failed',
            message: outboxId,
...
async function applyDeferralOrAbandon(db, outboxRef, currentRetry, kind, nowMs) {
...
//# sourceMappingURL=dispatch-sms-outbox.js.map