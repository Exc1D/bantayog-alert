...
import { BantayogError, logDimension, responderShiftHandoffDocSchema, } from '@bantayog/shared-validators';
...
const acceptSchema = z
    ...
    .strict();
export async function initiateResponderHandoffCore(db, input, actor, correlationId) {
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        if (existing.exists) {
            ...
            if (existingData?.fromUid === actor.uid &&
...
export async function acceptResponderHandoffCore(db, input, actor, correlationId) {
    if (actor.claims.accountStatus !== 'active') {
        log({
            severity: 'ERROR',
    ...
    const { result: cached } = await withIdempotency(db, { key: `acceptResponderHandoff:${actor.uid}:${input.idempotencyKey}`, payload: input }, async () => {
...
//# sourceMappingURL=responder-shift-handoff.js.map