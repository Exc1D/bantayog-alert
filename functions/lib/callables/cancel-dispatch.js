...
export async function cancelDispatchCore(db, deps) {
    ...
    const { result } = await withIdempotency(db, {
        ...
        if (!CANCELLABLE_FROM_STATES.includes(from)) {
            throw new BantayogError(BantayogErrorCode.FAILED_PRECONDITION, `Cannot cancel dispatch in status ${from} (allowed: ${CANCELLABLE_FROM_STATES.join(', ')})`);
        }
...
//# sourceMappingURL=cancel-dispatch.js.map