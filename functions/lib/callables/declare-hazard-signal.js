 ...
 */
export async function declareHazardSignalCore(db, input, actor) {
    ...
    const normalizedMunicipalityIds = validated.scopeType === 'province'
 ...
 */
export async function clearHazardSignalCore(db, input, actor) {
    if (actor.role !== 'provincial_superadmin') {
        throw new HttpsError('permission-denied', 'superadmin_required');
    }
...
export const clearHazardSignal = onCall({ region: 'asia-southeast1', enforceAppCheck: true }, async (request) => {
    if (!request.auth)
        throw new HttpsError('unauthenticated', 'sign-in required');
    const role = request.auth.token.role;
...
//# sourceMappingURL=declare-hazard-signal.js.map