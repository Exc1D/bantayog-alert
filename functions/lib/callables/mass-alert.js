...
function canActOnScope(actor, municipalityIds) {
    if (actor.claims.role === 'provincial_superadmin')
        return true;
    if (!actor.claims.municipalityId)
        return false;
    return municipalityIds.length === 1 && municipalityIds[0] === actor.claims.municipalityId;
}
export async function massAlertReachPlanPreviewCore(db, input, actor) {
    ...
    const route = total > MAX_DIRECT_ROUTE || municipalityIds.length > 1 ? 'ndrrmc_escalation' : 'direct';
...
export async function sendMassAlertCore(db, input, actor) {
    if (!ADMIN_ROLES.includes(actor.claims.role)) {
        return { success: false, errorCode: 'permission-denied' };
    }
    if (!canActOnScope(actor, input.targetScope.municipalityIds)) {
    ...
    const { result: cached } = await withIdempotency(db, { key: `send-mass-alert:${input.idempotencyKey}`, payload: input, now: () => Date.now() }, async () => {
        const serverPreview = await massAlertReachPlanPreviewCore(db, {
...
export async function requestMassAlertEscalationCore(db, input, actor) {
    if (!ADMIN_ROLES.includes(actor.claims.role)) {
        return { success: false, errorCode: 'permission-denied' };
    }
    if (!canActOnScope(actor, input.targetScope.municipalityIds)) {
    ...
    const { result: cached } = await withIdempotency(db, { key: `escalate-mass-alert:${input.idempotencyKey}`, payload: input, now: () => Date.now() }, async () => {
        const requestId = crypto.randomUUID();
        await db
            .collection('mass_alert_requests')
            .doc(requestId)
...
export async function forwardMassAlertToNDRRMCCore(db, input, actor) {
    if (actor.claims.role !== 'provincial_superadmin') {
        return { success: false, errorCode: 'permission-denied' };
    }
    try {
...
export const massAlertReachPlanPreview = onCall({ region: 'asia-southeast1', enforceAppCheck: process.env.NODE_ENV === 'production' }, async (request) => {
    const actor = requireAuth(request, ADMIN_ROLES);
    const input = z
        .object({
...
export const sendMassAlert = onCall({ region: 'asia-southeast1', enforceAppCheck: process.env.NODE_ENV === 'production' }, async (request) => {
    const actor = requireAuth(request, ADMIN_ROLES);
    const input = z
        .object({
...
export const requestMassAlertEscalation = onCall({ region: 'asia-southeast1', enforceAppCheck: process.env.NODE_ENV === 'production' }, async (request) => {
    const actor = requireAuth(request, ADMIN_ROLES);
    const input = z
        .object({
        message: z.string().min(1).max(1024),
    ...
    if (!input.success)
        throw new HttpsError('invalid-argument', input.error.message);
    const result = await requestMassAlertEscalationCore(adminDb, {
...
export const forwardMassAlertToNDRRMC = onCall({ region: 'asia-southeast1', enforceAppCheck: process.env.NODE_ENV === 'production' }, async (request) => {
...
//# sourceMappingURL=mass-alert.js.map