...
export async function createResponderCore(
  db: Firestore,
  deps: CreateResponderCoreDeps,
): Promise<{ uid: string; agencyId: string; availabilityStatus: 'available' }> {
  const correlationId = crypto.randomUUID()

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const { now: _now, ...idempotentPayload } = deps
  const { result } = await withIdempotency<
    Omit<CreateResponderCoreDeps, 'now'>,
    { uid: string; agencyId: string; availabilityStatus: 'available' }
  >(
    db,
    {
    ...
    async () => {
      ...
      const callerAgencyId = deps.actor.claims.agencyId as string | undefined
      ...
      try {
        ...
        throw new BantayogError(
          ...
          `Phone already in use by user ${existing.uid}`,
        )
      } catch (err: unknown) {
        if (err instanceof BantayogError) throw err
        const code =
      ...
      const municipalityId =
        deps.municipalityId ?? (deps.actor.claims.municipalityId as string | undefined)
      ...
      return { uid, agencyId: deps.agencyId, availabilityStatus: 'available' as const }
    },
  )
