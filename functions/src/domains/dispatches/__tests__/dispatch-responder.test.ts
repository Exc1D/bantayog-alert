    ...
    async () => {
      await testEnv!.withSecurityRulesDisabled(async (ctx) => {
        ...
        const dispatchEvents = await db
          ...
          .get()
        const dispatchEventData: Record<string, unknown>[] = dispatchEvents.docs.map(
          (doc: { data: () => Record<string, unknown> }) => doc.data(),
        )
        expect(dispatchEventData).toHaveLength(2)
        expect(
          dispatchEventData.filter((event) => event.from === null && event.to === 'pending'),
        ).toHaveLength(1)
        expect(
          dispatchEventData.filter((event) => event.type === 'notification_attempted'),
        ).toHaveLength(1)
      })
    },
  )
