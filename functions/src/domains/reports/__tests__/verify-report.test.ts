...
describe('verifyReportCore', () => {
  ...
  itif(available)(
    'advances awaiting_verify → verified with scrubbed copy and makes the report public',
    async () => {
      await testEnv!.withSecurityRulesDisabled(async (ctx) => {
        const db = ctx.firestore() as any
  ...
  itif(available)(
    'advances awaiting_verify → verified without scrubbed copy but keeps the report internal',
    async () => {
      await testEnv!.withSecurityRulesDisabled(async (ctx) => {
        const db = ctx.firestore() as any
        const { reportId } = await seedReportAtStatus(db, 'awaiting_verify', {
