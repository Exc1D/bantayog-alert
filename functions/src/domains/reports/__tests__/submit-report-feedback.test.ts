...
describe('submitReportFeedbackCore', () => {
  ...
  itif(available)(
    'rejects missing report_private with NOT_FOUND (enumeration protection)',
    async () => {
      await testEnv!.withSecurityRulesDisabled(async (ctx) => {
        const db = ctx.firestore() as any
        const now = Timestamp.fromMillis(1713350400000)
  ...
  itif(available)(
    'supports A → B → A correction cycle (idempotency removed for upsert semantics)',
    async () => {
      await testEnv!.withSecurityRulesDisabled(async (ctx) => {
        const db = ctx.firestore() as any
        const { reportId } = await seedReportAtStatus(db, 'resolved', {
