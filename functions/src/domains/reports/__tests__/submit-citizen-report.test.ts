...
function baseInput(overrides: Partial<Parameters<typeof submitCitizenReportCore>[1]> = {}) {
  return {
...
describe('submitCitizenReportCore', () => {
  itif(available)(
    'materializes a citizen report triptych directly from callable input',
    async () => {
      await env!.withSecurityRulesDisabled(async (ctx) => {
  ...
  itif(available)('rejects an existing publicRef when the secret hash differs', async () => {
    await env!.withSecurityRulesDisabled(async (ctx) => {
      ...
      await expect(
        submitCitizenReportCore(
          db,
          baseInput({ idempotencyKey: 'idem-conflict', secretHash: 'b'.repeat(64) }),
        ),
      ).rejects.toThrow(/publicRef already exists/i)
    })
  })
