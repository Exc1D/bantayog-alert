...
if (!firestoreAvailable) {
  console.warn(
    '[adversarial-audit] Firestore emulator unavailable at 127.0.0.1:8081; tests will skip.',
  )
}
if (!rtdbAvailable) {
...
async function seedReportAdmin(reportId: string, status: string): Promise<void> {
...
async function seedResponderAdmin(uid: string, municipalityId = 'daet'): Promise<void> {
  await adminDb
...
describe('Adversarial Audit — Proof of Concept', { timeout: 120000 }, () => {
  itif(emulatorAvailable)(
    'BUG #1: Redispatch Deadlock — cannot dispatch new responder if report is "assigned"',
    async () => {
      ...
      await expect(
        dispatchResponderCore(adminDb, adminRtdb, {
          reportId,
  ...
  itif(emulatorAvailable)(
    'BUG #2: Multi-Dispatch Collision — old dispatch regresses report status',
    async () => {
  ...
  itif(emulatorAvailable)(
    'FIXED: Idempotency — retry with different Timestamp returns cached result (fromCache=true)',
    async () => {
  ...
  itif(emulatorAvailable)(
    'FIXED: PII Scrubbing — verifyReport callable now accepts scrubbedDescription',
    async () => {
