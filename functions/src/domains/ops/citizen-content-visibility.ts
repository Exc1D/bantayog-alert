...
const SURFACES = ['feed', 'alerts'] as const
...
function collectionForSurface(surface: CitizenContentSurface): string {
  return surface === 'alerts' ? 'alerts' : 'situation_updates'
}

function municipalityIds(data: Record<string, unknown>): string[] {
...
function assertActorCanModerate(
  deps: SetCitizenContentVisibilityDeps,
...
export async function setCitizenContentVisibilityCore(
  db: Firestore,
  deps: SetCitizenContentVisibilityDeps,
): Promise<{
  surface: CitizenContentSurface
...
export const setCitizenContentVisibility = onCall(
  ...
  async (request: CallableRequest<unknown>) => {
    ...
    const actorClaims: SetCitizenContentVisibilityDeps['actor']['claims'] = {}
