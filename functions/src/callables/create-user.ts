...
export async function createUserCore(
  db: Firestore,
  deps: CreateUserCoreDeps,
): Promise<{ uid: string; role: string; accountStatus: 'active' }> {
  const correlationId = crypto.randomUUID()

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const { now: _now, ...idempotentPayload } = deps
  const { result } = await withIdempotency<
    Omit<CreateUserCoreDeps, 'now'>,
    { uid: string; role: string; accountStatus: 'active' }
  >(
    db,
    {
    ...
    async () => {
      ...
      return { uid, role: deps.role, accountStatus: 'active' as const }
    },
  )
