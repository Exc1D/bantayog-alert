...
export async function requestProvincialEscalationCore(
  db: Firestore,
  deps: RequestProvincialEscalationCoreDeps,
): Promise<{ escalationId: string; status: 'pending' }> {
  const correlationId = crypto.randomUUID()

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const { now: _now, ...idempotentPayload } = deps
  const { result } = await withIdempotency<
    Omit<RequestProvincialEscalationCoreDeps, 'now'>,
    { escalationId: string; status: 'pending' }
  >(
    db,
    {
    ...
    async () => {
      ...
      return db.runTransaction(async (tx) => {
        ...
        return { escalationId, status: 'pending' as const }
      })
    },
  )
