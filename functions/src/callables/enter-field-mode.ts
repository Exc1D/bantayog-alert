...
export const enterFieldMode = onCall(
  ...
  async (req: CallableRequest<unknown>) => {
    ...
    const authTime =
      typeof rawAuthTime === 'number' && isFinite(rawAuthTime)
        ? rawAuthTime
        : typeof rawAuthTime === 'string' && isFinite(Number(rawAuthTime))
          ? Number(rawAuthTime)
          : 0
    const muni =
      typeof actor.claims.municipalityId === 'string' && actor.claims.municipalityId.length > 0
        ? actor.claims.municipalityId
        : undefined
    const claims: FieldModeActorClaims = {
      role: typeof actor.claims.role === 'string' ? actor.claims.role : '',
      accountStatus:
        typeof actor.claims.accountStatus === 'string' ? actor.claims.accountStatus : '',
...
export const exitFieldMode = onCall(
  ...
  async (req: CallableRequest<unknown>) => {
    const actor = requireAuth(req, ['municipal_admin', 'agency_admin', 'provincial_superadmin'])
    const actorClaims: { role: string; accountStatus: string; municipalityId?: string } = {
      role: typeof actor.claims.role === 'string' ? actor.claims.role : '',
      accountStatus:
        typeof actor.claims.accountStatus === 'string' ? actor.claims.accountStatus : '',
    }
    const muni =
      typeof actor.claims.municipalityId === 'string' && actor.claims.municipalityId.length > 0
        ? actor.claims.municipalityId
        : undefined
    if (muni) actorClaims.municipalityId = muni
    try {
