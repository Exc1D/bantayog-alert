...
function readClaimString(
  ...
  key: 'role' | 'municipalityId' | 'agencyId',
) {
...
function toIncidentPayload(
  ...
  opsData: FirebaseFirestore.DocumentData,
): ScopedOperationsMapIncidentPayload | null {
  ...
  const submittedAt =
    typeof reportData.submittedAt === 'number'
  ...
  const updatedAt =
    typeof reportData.updatedAt === 'number'
  ...
  return {
    ...
    report: {
      municipalityId:
        typeof reportData.municipalityId === 'string' ? reportData.municipalityId : 'unknown',
...
export async function listScopedOperationsMapCore(
  ...
  const accountStatus =
    typeof actor.claims.accountStatus === 'string' ? actor.claims.accountStatus : undefined
  ...
  return {
    incidents: incidents.filter(
      (item): item is ScopedOperationsMapIncidentPayload => item !== null,
    ),
  }
}
