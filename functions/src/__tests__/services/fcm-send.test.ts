...
describe('sendFcmToResponder', () => {
  ...
  it('removes invalid tokens on failure', async () => {
    mockGet.mockResolvedValueOnce({ exists: true, data: () => ({ fcmTokens: ['valid', 'invalid'] }) })
    mockSendEachForMulticast.mockResolvedValueOnce({
      responses: [
        { success: true },
    ...
    const arrayRemoveSpy = vi.spyOn(FieldValue, 'arrayRemove').mockReturnValue('array_remove_mock' as any)
