...
describe('mass_alert_requests rules', () => {
  ...
  it('denies cross-municipality create', async () => {
    ...
    await assertFails(
      setDoc(doc(db, 'mass_alert_requests', 'req-cross'), {
        ...baseAlert('queued'),
  ...
  it('denies missing requestedByMunicipality', async () => {
    ...
    delete (alertWithoutMuni as Record<string, unknown>).requestedByMunicipality
  ...
  it('denies missing status field', async () => {
    ...
    delete (alertWithoutStatus as Record<string, unknown>).status
  ...
  it('denies status forwarded_to_ndrrmc', async () => {
    ...
    await assertFails(
      setDoc(doc(db, 'mass_alert_requests', 'req-forwarded'), baseAlert('forwarded_to_ndrrmc')),
    )
  })
  ...
  it('denies muni admin update', async () => {
    ...
    await testEnv.withSecurityRulesDisabled(async (ctx) => {
      await setDoc(
        doc(ctx.firestore(), 'mass_alert_requests', 'req-for-update'),
        baseAlert('queued'),
      )
    })
    ...
    await assertFails(
      setDoc(
        doc(adminDb, 'mass_alert_requests', 'req-for-update'),
  ...
  it('allows superadmin create queued', async () => {
    ...
    await assertSucceeds(
      setDoc(doc(db, 'mass_alert_requests', 'req-super'), {
        ...baseAlert('queued'),
  ...
  it('denies cross-municipality read', async () => {
    ...
    const otherDb = authed(
      testEnv,
      'other-admin',
      staffClaims({ role: 'municipal_admin', municipalityId: 'pasacao' }),
    )
    await seedActiveAccount(testEnv, {
      uid: 'other-admin',
      role: 'municipal_admin',
  ...
  it('allows superadmin update', async () => {
    ...
    await testEnv.withSecurityRulesDisabled(async (ctx) => {
      await setDoc(
        doc(ctx.firestore(), 'mass_alert_requests', 'req-super-update'),
        baseAlert('queued'),
      )
    })
    ...
    await assertSucceeds(
      setDoc(
        doc(superDb, 'mass_alert_requests', 'req-super-update'),
        { status: 'sent' },
        { merge: true },
      ),
    )
  ...
  it('denies requestedByUid mismatch', async () => {
    ...
    await assertFails(
      setDoc(doc(otherDb, 'mass_alert_requests', 'req-uid-mismatch'), {
        ...baseAlert('queued'),
  ...
  it('allows superadmin update status only', async () => {
    await testEnv.withSecurityRulesDisabled(async (ctx) => {
      await setDoc(
        doc(ctx.firestore(), 'mass_alert_requests', 'req-status-update'),
        baseAlert('queued'),
      )
    })
    const superDb = authed(testEnv, 'super-admin', staffClaims({ role: 'provincial_superadmin' }))
    await assertSucceeds(
      setDoc(
        doc(superDb, 'mass_alert_requests', 'req-status-update'),
        { status: 'sent' },
        { merge: true },
      ),
    )
  ...
  it('denies extra field injection', async () => {
    ...
    await assertFails(
      setDoc(doc(db, 'mass_alert_requests', 'req-extra'), {
        ...baseAlert('queued'),
  ...
  it('denies missing required fields', async () => {
    ...
    await assertFails(
      setDoc(doc(db, 'mass_alert_requests', 'req-minimal'), {
        status: 'queued',
      }),
    )
  })
  ...
  it('allows estimatedReach field', async () => {
    ...
    await assertSucceeds(
      setDoc(doc(db, 'mass_alert_requests', 'req-reach'), {
        ...baseAlert('queued'),
  ...
  it('allows targetType municipality', async () => {
    ...
    await assertSucceeds(
      setDoc(doc(db, 'mass_alert_requests', 'req-target'), {
        ...baseAlert('queued'),
  ...
  it('denies requestedByMunicipality null', async () => {
    ...
    await assertFails(
      setDoc(doc(db, 'mass_alert_requests', 'req-null-muni'), {
        ...baseAlert('queued'),
  ...
  it('denies non-string status', async () => {
    ...
    await assertFails(
      setDoc(doc(db, 'mass_alert_requests', 'req-status-number'), {
        ...baseAlert('queued'),
        status: 123,
      }),
    )
