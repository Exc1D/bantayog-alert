...
function validPayload(capturedAt: number) {
  return {
    capturedAt,
    lat: 14.0931,
    lng: 122.9544,
...
describe('responder_locations write', () => {
  ...
  it('blocks write when capturedAt is older than 10 minutes', async () => {
    ...
    await assertFails(
      db.ref(`responder_locations/${RESPONDER_UID}`).set(validPayload(Date.now() - 700_000)),
    )
  })
  ...
  it('blocks a responder from writing to another responder node', async () => {
    ...
    await assertFails(
      db.ref(`responder_locations/${OTHER_RESPONDER_UID}`).set(validPayload(Date.now())),
    )
  })
...
describe('responder_index client access', () => {
  ...
  it('blocks any authenticated client write on responder_index', async () => {
    ...
    await assertFails(
      db.ref(`responder_index/${RESPONDER_UID}`).set({ municipalityId: 'injected' }),
    )
  })
})
